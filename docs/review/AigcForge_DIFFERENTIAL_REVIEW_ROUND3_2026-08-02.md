...
- `backfill`：逐行 `Identifier.ascending("task")` 生成 tsk_ id（无 hex(randomblob)）；created_at/updated_at/position 保留；未知 status/priority 归一 pending/medium；**幂等实证**：journal 跟踪重跑 no-op；`applyOnly` up+journal 同事务，中途失败整体回滚；同批 id 严格递增；
