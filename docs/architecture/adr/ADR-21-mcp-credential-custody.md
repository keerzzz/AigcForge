...
- **rotation** 复用既有：`Credential.update(id, { value })`（`credential.ts:43`）已是唯一写入口，OAuth 刷新走 `Integration` 的 `refresh` 回调（`integration.ts:89`）。绑定层持 ref 不持材料，**所以轮换对绑定完全透明**——这正是 opaque ref 的收益，不需要新机制。
...
- **止血 1（M3 范围，成本极低）**：数据库文件创建后 chmod `0o600`，与既有两个文件存储对齐。这不是加密，是把**已有的**保护级别补齐到一致。
