...
### 19.1 2026-08-19 HTTP 契约修订

M1 实现复核确认：`session.children` 与 `session.context` 对能力客户端属于 Custom 会话只读接口；Custom 父会话允许通过 HTTP fork 路由到 V2 `create({ parentID })` 并复制不可变 Snapshot。V1 同步 prompt/command/shell 对 Custom 返回 typed `UnsupportedProductModeError`，且 `POST /api/session/custom` 必须携带 `x-aigcfroge-capabilities: product-mode-custom-v1`。
