...
> 状态：Proposed v1.1，已完成协议一致性复核，待 Product/Core/App/Security 正式评审
> 日期：2026-08-16
...
Profile 是用户可保存和复用的装配说明。目标是作为第八类资产 `custom-profile`，由 Chat 管理、Custom 消费；在本 ADR Accepted、AssetKind owner、目录/格式和 typed 事务契约获批前，这仍是产品方向，不是已实现资产类型。
...
Draft 阶段允许用户装配和修改。首次提交前，Composition Resolver 将 Profile 和所有引用解析为不可变快照，Session 从此只使用该快照：
...
- 有效工具目录、Tool Presentation 和 digest。
- 有效权限摘要、Location policy 和 approval facts 的 digest。
- 缺失、冲突、降级和外部凭证引用的诊断结果。

快照是 Session 恢复和模型可见输入的真源之一。资产文件后来变化，不改变已有快照。

快照不替代 Session V2 Context Epoch：Composition Snapshot 保存组合、版本、绑定和能力事实，Context Epoch 保存实际展示给模型的完整系统上下文。权限摘要或 digest 只用于审计，每次工具执行仍由当前 canonical PermissionV2 重新判断。

#### 2.5 经协议复核确认的六项范围裁决

1. `custom-profile` 作为第八类资产的方向成立，但受 ADR 接受与独立 owner contract 约束。
2. M1 运行拓扑固定为 `meta` 根 Session + 一个用户 Agent 委派目标，不是“一个 Agent 直接拥有根会话”。
3. M1 只解析当前 Location 的项目资产；全局资产、跨项目引用和隐式全局工作区均不开放。
4. M1 完全不运行 Runtime Extension；M4 才开放已安装、验证、审批、版本固定且可停止/隔离/回滚的 Trusted Extension。
5. 删除 Profile 后历史 Session 与 Snapshot 始终可查看；是否继续执行取决于冻结内容和精确运行依赖，缺失时必须阻断，升级通过 fork/new Session。
6. 应用级审批入口属于 M3；入口可跨页面发现请求，但授权事实只能明确绑定 once、Session 或 Location，不能默认为应用级永久信任。
...
它不执行工具、不替代 `ToolRegistry`、不直接加载任意 Plugin 代码、不成为第二套权限判断器。工具执行仍经当前 canonical ToolRegistry settlement，用户审批仍经当前 Permission/Policy。
  ...
  -> CompositionResolver
      -> CompositionPlan (可预览、可诊断)
          -> Permission/Policy evaluation
              -> CompositionSnapshot
                  -> SessionV2 / Meta dispatch / ToolRegistry
```
...
- Profile 的具体文件扩展名和完整 Schema。
- Profile 历史版本是否写入文件、数据库或内容寻址存储。
- MCP 客户端的具体 Session/Location 注册实现。
- M3 grant 存储是否扩展现有 PermissionSaved，或由新的 scoped grant owner 承担。
- Plugin 签名格式、远程市场和跨设备分发。
- Code Presentation 的 TypeScript runtime 选择。
