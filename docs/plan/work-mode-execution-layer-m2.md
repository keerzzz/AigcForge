...
| 路由协议：Work 产出 -> Chat 资产注册（预填 propose_*_asset 入参） | **不经 LLM propose 工具**（那是 Chat orchestrator 的链路）。Work UI 直接构造 CandidateInfo + setProposeCandidate，复用 Chat 右栏审查 UI（见 §3.3 D3） |
