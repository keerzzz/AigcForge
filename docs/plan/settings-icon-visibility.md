...
3. Expose signals and setter methods inside `SettingsProvider` init:
   ```typescript
   const showLeftSidebarToggle = withFallback(() => store.general?.showLeftSidebarToggle, defaultSettings.general.showLeftSidebarToggle)
   const showRightSidebarToggle = withFallback(() => store.general?.showRightSidebarToggle, defaultSettings.general.showRightSidebarToggle)
   ```
