...
> 状态：**Phase A–G 全部交付** —— Phase A（`7a2804624`）、Phase B（`99dce8906`）、Phase D（`38d82e2b3`）、Phase F0（`f66f93d8c`）、Phase C（合并 `f21797e08`）、Phase E（合并 `e0fe0e438`，证据更正 `2c9e779b9`）、Phase F1–F5（合并 `6a38871aa` / `7d4ef3373` / `6a5714e2c` / `660301279`）、**Phase G**（合并 `4278b45f7`，2026-08-27）均已交付、经独立复审整改并合入本地 main。[ADR-21](../architecture/adr/ADR-21-mcp-credential-custody.md) 现为 **Accepted v1.2**（2026-08-25 Slice 2 发现 rebind 缺陷后修订）。**当前无可开工 Phase**；剩余动作是合入前整改 + 开 PR
> 执行提示词：[Custom Mode M3 执行提示词](prompt-custom-mode-m3-mcp-approval.md)（写于 2026-08-24 时收窄为纯 Phase C，其 Phase C 段已随交付过期，勿作为开工依据）
> 分析基线：§0 写于 `main@a11b50020`（2026-08-22），已按 Phase A/B/D/F0 复核逐条更正；**§0 之后的 Phase C/E/F/G 交付使 §0 多条事实过期，读 §0 前先读对应 Phase 的「交付事实」小节**。交付基线为本地 `main@bb46b3f83`，领先 origin/main 87 提交（M3 全部结束后统一开一个 PR）。测试基线（2026-08-27 合并前整改后复跑）：core 2206 pass / 2 skip / 0 fail（259 文件）、schema 147/0、app 单测 947/0、aigcfroge 3240 pass / 22 skip / 0 fail、exerciser coverage 289/289
...
**所有 Phase 均已交付**（分支与合并哈希见状态头）。Phase C 由 `mcp-connection` 交付：**G3-3 于 2026-08-24 通过**（[ADR-21](../architecture/adr/ADR-21-mcp-credential-custody.md)，现 Accepted v1.2），Slice 0 独立事实复核作为不可跳过的前置已执行并据其结论把 ADR 修订到 v1.1，Slice 2 又发现 rebind 缺陷修订到 v1.2 —— 两次都是执行方交付后由复审方红证抓出，而非起草方自查发现。Phase E 消费 Phase C 交付的连接实体，Phase F 的产品界面已在 F1–F5 定案。
...
> **Phase D 两项 R6 事项的历史记录**（technical-debt §3.1 已独立登记；下列 attended ceiling 事项已由现有 `effective.ts` 实现，后续只保留回归验证）：
>
> 1. **attended custom 的资产自授权限尾部 allow**（[ADR-20 §2.6](../architecture/adr/ADR-20-scoped-grant-model.md) attended 条目，裁定为重写成 `ask`）。该规则已经在 `effective.ts:59-67` 实现：custom 资产来源的非只读 `allow` 一律改判 `ask`，不依赖 `attended`；只读白名单保持 `allow`，显式 deny 的位序仍优先。`savedApprovals` 在下游独立追加为 `allow`，保留用户真实点过 always 的显式授权，符合 ADR 裁定。后续阶段只做回归验证和审批展示，不重复实施。
>
...
> 事实校准（§0.1，**2026-08-26 恢复**）：V2 pending/list/reply API 与 `permission.v2.*` 事件都已存在并已挂载，真源在 `packages/server` 而不是 `packages/aigcfroge/src`：`GET /api/permission/request`（`v2.permission.request.list`）、`GET /api/session/:sessionID/permission`（`v2.session.permission.list`）、`POST /api/session/:sessionID/permission/:requestID/reply`（`v2.session.permission.reply`）都使用 `PermissionV2.Service`，并带 `OpenApi.annotations identifier`。此前把检索范围限于 `packages/aigcfroge/src` 后宣布端点不存在是错误。客户端消费仍为零，故本 Phase 的重心是 SDK 与 L4 审批中心，而非新建 L3 endpoint。三个必须处理的陷阱：① custom Session 的事件对没带 `product-mode-custom-v1` 能力头的连接会被 SSE 过滤；② app 侧的浏览器 auto-accept 不是服务端 grant，不能与 `ScopedGrant` 混为一谈；③ responder 过报已由 F1 修复，responder 事实现在按 capability 与 Location 对齐。
...
- 实现分支现状（全部合入本地 `main`，未推送）：`mcp-scope-adr`（Phase A）、`mcp-registration`（Phase B）、`scoped-grants`（Phase D）、`approval-preflight`（F0）、`mcp-connection`（Phase C Slice 0-4）、`mcp-composition`（Phase E）、`ceiling-display` 与 `approval-client` 与 `approval-scopes`（F1-F4）、`global-event-harness`（F1 遗留回归修复）、`auto-accept-boundary`（F5）、Phase G（故障注入与灰度，合并 `4278b45f7`）。**Phase 全部完成，无剩余 Phase。** F5 的产出是把「浏览器 auto-accept 只应答 V1 ask」从事件名巧合改成类型强制的边界（放宽守卫即 typecheck 失败，双向红证），因此 custom/MCP 审批不可能被浏览器自动放行，**F5 不构成 Phase G 的前置**；V1 面 `base64(directory)/*` 通配键的三种处置属产品裁定，已转 [technical-debt](../technical-debt.md) §4。
