...
> **状态**：已审批（第 4 版，2026-09-04）；**实施完成（2026-09-06，分支 `dogfood-remediation`，提交按 §7 提交表逐 slice 落地）**。
...
| R7  | §2.2「V1 已把拒绝转成 error part，但错误消息与终止原因不可见/不够可恢复」 | 从真实入口实测（`processor-effect.test.ts` 新增用例）：V1 三个权限错误类都有 `message` 覆写、`CorrectedError` 原样带 feedback，`failToolCall` 写进 `state.error`；并且 V1 **按错误分档轮次**——`processor.ts:241` 只对 `RejectedError` 置 `ctx.blocked`，故 corrected→`continue`、rejected→`stop` | P1-PERMISSION-DENY 的 core 侧是 **V2-only 回归**，不是 V1 缺功能；V1 侧只补行为钉子，不改生产代码                                                          |
| R8  | 报告的「Shell 卡片失败但没有错误说明」归因于运行时/core                   | 第三个位置，计划未列：`session-ui/tool-error-card.tsx` 折叠展示，且 subtitle 由「按 `": "` 切分、取冒号前、无冒号则 `Failed`」派生。V1 那句无冒号→只显示「Shell — Failed」；新 V2 那句也会把 feedback 切掉                                                                                       | 该缺陷与运行时无关，两条运行时同受影响；已在 session-ui owner 修为「首行摘要 + 展开看全文」，并以真实路由 e2e 钉住                                         |
| R9  | （无对应旧结论）                                                          | V2 无法表达 V1 的轮次分档：五种 outcome 统一以 `ToolFailure` 离开 leaf，这是 `Tool.make` 的 execute 错误通道唯一允许的形状，runner 只看到一种                                                                                                                                                    | 记入 `docs/technical-debt.md` §4，标为 **V1 退役前**必须闭环——退役或默认切 V2 都会让分档静默消失                                                           |
...
### 5.1 S3b 实施期修订（2026-09-05 实测）

| 计划表述                                                                | 实测                                                                                                                                                                                                                                                                       | 处置                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 「V1 现有 header/chunk 实现和真实用户入口跑同组关键语义」需新建 RED-3   | V1 已有 `packages/aigcfroge/test/provider/header-timeout.test.ts` 六例行为测试，覆盖「头到达后不因慢 body 中断」「SSE 停顿抛 ResponseStreamError」「头不来即中断」「非 OpenAI 需显式开启」「`false` 可关」「OpenAI 默认 10,000」                                           | 不新建 V1 测试；V1 无需改动，parity 由既有测试背书。V2 不 import V1 常量，实现独立                                                                                                                                                                                                                                                                             |
| 默认值「采用已有 V1 验证数值 10,000/60,000」                            | 初稿实施时判为「不加默认、记债」，**该判断已推翻**：V2 是优先运行时，让它比 V1 弱等于随 V1 退役丢保护，且 `极致减法` 的第一优先级是复用已验证行为而不是重新论证                                                                                                            | **已加默认**：`chunkTimeout` 全局 60,000ms、`headerTimeout` 仅 `@ai-sdk/openai` 10,000ms，与 V1 的作用域逐条对齐（V1 chunk 用 `?? PROVIDER_CHUNK_TIMEOUT_DEFAULT` 全局兜底、header 由 `openai` 注册项 seed）。只复用数值与作用域，不 import V1 常量。`false` 永不被默认值覆盖。停止条件已实测：core 2310 pass / 0 fail、schema 206 / 0，无测试依赖「永不超时」 |
| 「schema settings 能 decode/typecheck 三档，保留开放 provider options」 | `Provider.Settings` 用 `Schema.StructWithRest` 具名三档 + 开放 rest；`0` 被 `PositiveInt` 拒绝，`false` 为显式关闭档                                                                                                                                                       | 已实现。`core/src/provider.ts` 的 `MutableApi` 随之从 `settings: any` 收窄为 `MutableSettings`                                                                                                                                                                                                                                                                 |
| 「request body 同名字段不能静默覆盖 provider transport deadline」       | 旧实现从 merge 后的 option bag 读 deadline，而 `model.request.body` 后 spread，故 body 的 `timeout` 会改写整个 provider 的传输期限                                                                                                                                         | 已修：先从 settings 读、再 merge。可观测断言：provider 5s + body 1ms，只settle-on-abort 的 fake fetch 保持未决；把读取顺序改回即报 `aborted`                                                                                                                                                                                                                   |
| S3b 前置「plugin 模块循环」                                             | 真实成因不是 `plugin.ts ↔ plugin/internal.ts` 双向 import（二者无直接环），而是模块求值期就构造 layer 图：`internal.ts ↔ config/plugin/*.ts` 六个环 + `plugin.ts → plugin/host.ts` + `plugin.ts → skill.ts → permission.ts → session.ts → location-layer.ts → plugin.ts` | 已修：`locationLayer` 改为 `Layer.suspend` 惰性构造，环内任一入口都不再读到未初始化的 binding。全量套件此前已绿，故这是「窄选择运行才炸」的隐性门禁缺口                                                                                                                                                                                                        |

### 5.2 S4 实施期修订（2026-09-05 实测）

| 计划表述                                                                                       | 实测                                                                                                                                                                                                                                                              | 处置                                                                                                                                                                       |
| ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 「已明确仍未门控的 slot 内专属资源是 Assistant」，指 `assistant-dashboard.tsx` 五个 `useQuery` | 少算一个文件：`components/assistant-nav-tree.tsx` 还有四个（pending/memory/kb/dangling），由 `assistant-feature-sidebar.tsx:25` 挂进 Assistant 侧栏槽；它同时被 `assistant-session-sidebar.tsx:79` 用在会话页                                                     | 两个文件共九个 query 一起加 `enabled: slotActive()`。会话页无 slot provider，`useModeSlotActive` 返回 always-active，故同一个门在两处都对                                  |
| 隐藏槽泄漏包含 `/schedule/pending`                                                             | **不是泄漏**：`components/mode-switcher.tsx:24-31` 以 60s 间隔轮询它，用于 Assistant 导航项的待办角标——它的意义恰恰在于「你在别的模式时也要看到」。三处共用同一 query key，所以只发一次                                                                           | 按「共享调用 + 具名 owner」单独记账：断言它恰好 1 次，而不是断言 0 次。这是计划 §2.4 要求的第二种分类，落在 ModeSwitcher 而非 Workspace                                    |
| `ModeWorkspace` 顶层 Chat 资源需分类为「Chat 专属」或「共享预热」                              | **Chat 专属**，由调用图定：`ModeWorkspaceAssetCtx` 只有一个消费者 `ChatAssetWorkbenchMain`（`mode-workspace-slots.tsx:433`）                                                                                                                                      | 用**闩锁**而非实时 gate：首次进入 Chat 才启动，之后保持。实时 gate 会在离开 Chat 时清空资源，等于丢掉 render-all 要保的状态并在每次回来重新拉                              |
| 「最终可见 loading/error owner 放在 ModeWorkspace 槽边界；`layout.tsx` 只做最小全局兜底」      | S0 曾实测「pending 期槽根本没挂载，所以槽级 fallback 不可能是全部答案」并记为约束。加了槽级边界后该事实反转：槽会挂载，只有等待中的那个槽内容被替换。**计划原判是对的，但 S0 记录的理由是错的**                                                                   | S0 基线断言已改为断后修行为。`layout.tsx` 的 fallback 经实测**当前不可达**（无任何 route 组件在槽边界之上读 resource），注释写明它是保险而非有测试覆盖的路径——不假装有覆盖 |
| 「不得让整个 `<main>` 变成空白」                                                               | 比这更糟：`/workflow-asset` 返 500 时，`mode-workspace-slots.tsx:673` 的 resource 被 SDK interceptor 抛出，穿透所有边界打到 `app.tsx` 顶层 `ErrorBoundary`，**整个应用**变成「Something went wrong」。per-kind settle 只护住了 chat 的七类资产，work 这条是直接读 | 每槽加 `ErrorBoundary`，失败只留在该槽内并给 Retry；其余四槽保持挂载                                                                                                       |
