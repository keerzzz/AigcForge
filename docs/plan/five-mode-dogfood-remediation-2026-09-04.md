...
| 默认值「采用已有 V1 验证数值 10,000/60,000」                            | V1 的 header 默认按 provider 家族分档（仅 OpenAI 系），chunk 为全局 60,000；V2 provider 集合不同，照搬即臆想                                                                                                                                                               | **不加默认**，记债（`docs/technical-debt.md` §4）。理由：默认值会杀掉合法长轮次；S3a 已在客户端 60s 给出停滞态与出口，报告症状在用户路径上已闭环，服务端 deadline 属加固 |
