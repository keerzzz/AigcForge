...
| S2（含入口边界） | **已实施并审批通过** | 用户裁决取 B（通用 kernel）并批准「部分成功变整体失败」的产品语义 + 客户端同批迁移。`54f665d5a` 内核（先校验后提交、selection 幂等、一次 wake、整体 uninterruptible）→ `1aa5434d4` `EventV2.publishBatch`（单事务提交 N 个 durable 事件，唤醒与通知全部延到提交之后 —— 因为 `commitDurableEvent` 在自身事务返回时就通知，所以「包一层外层事务」是错的）→ `9dd4851d6` canonical `session.prompt` 与 legacy `prompt_async` 一次携带 selection → `aea8c31cc` SDK 再生成（238 path 不变，纯增量）→ `d4bc786dd` app/TUI handoff 退掉 switch-then-send 两段 dance。范围裁决记录：入口边界分析推翻了「只修 admitCanonical」的收窄方案 —— canonical API 原本在结构上强迫客户端自己拼两次调用，服务端 kernel 救不了；同时「把五种 admission 合成一个函数」被否，归并的是机制（事务 + 通知时序）不是签名。剩余：探针 P4 的跨连接守卫测试（已登记债） |
