...
| §3 Custom Mode 平台（PR #33） | ADR-17 评审 + Roadmap M0-M5；§3.1 Custom M2 遗留 10 项；§3.2 Custom M3 Phase B/D 遗留 + ADR-21 排除项 7 项 | M0/M1/M2 已完成并合入（M2 = PR #46 / `a11b50020`）；M3 Phase A/B/D 已交付，Phase C 待 ADR-21 裁定；M4-M5 远期 |
...
| M3 | MCP 与审批 | scoped registration、凭证、健康、统一审批入口（含 once/Session/Location grant model） | M2 + Tool Registry 扩展 | 进行中：G3-1（[ADR-19](architecture/adr/ADR-19-mcp-scoped-registration.md) Accepted v1.0）与 G3-2（[ADR-20](architecture/adr/ADR-20-scoped-grant-model.md) Accepted v1.1）已批准，Phase A 已合入 `main`（`7a2804624`）；Phase B（placement + MCP 命名/冲突 owner）与 Phase D（ScopedGrant + attended ask 天花板）均已交付并经独立复审整改。**Phase C 待 G3-3 裁定**（[ADR-21](architecture/adr/ADR-21-mcp-credential-custody.md) 已起草，状态 Proposed，由复审方起草故不得自批），Phase B/D 遗留与 Phase C/F 输入见 §3.2。计划 [custom-mode-m3-mcp-approval.md](plan/custom-mode-m3-mcp-approval.md)，执行提示词 [prompt-custom-mode-m3-mcp-approval.md](plan/prompt-custom-mode-m3-mcp-approval.md) |
...
| `ScopedGrantStore.issue` 每次签发都清扫全表已结算行，settled grant 不可查 | core | `issue()` 无条件 `DELETE` 全部 `consumed_at`/`revoked_at` 非空与已过期行（`grant/store.ts`），理由是「活状态表，历史留在 durable 事件」。这本身与 ADR-20 §2.4 一致（`grant.updated` 是记录源），且不丢数据；但**代价是任何「按 grant 查历史」的读路径都查不到**——审批中心要列「最近使用/已撤销的授权」时只能改读事件流并自行折叠。它还让「查一个已消费 grant」成为常规路径（Phase D 复审据此修掉了 `toInfoSafe` 的 defect）。**刻意不在 Phase D 定保留策略**：保留窗口（按时间/按条数/不清扫+加索引）取决于审批中心要展示什么，现在拍就是臆想业务。根治：与 Phase F 的审批中心一并定 retention，并据此决定是否给 `scoped_grant` 加索引（当前清扫使表极小，无索引不构成性能问题） | TBD | **Custom M3 Phase F 开工时**（与审批中心历史视图同一 slice） |
| 资产导入/apply 不警示通配 allow 声明（自 §3.1 移入，触发条件改判） | core / app / schema / sdk | 原登记于 §3.1 并标注「Phase D 开工时」，Phase D **未实现**。复审重新定界后移入本节并改触发条件：它不是一处「解码后加个判断」，而是跨 L1/L2/L3/L4 的功能——`AgentAssetService.propose`/`apply`（`agent-asset-service.ts:146`/`:198`）要在 `parseAgentAssetConfig` 解码后产出诊断，就得改 `ProposeResult` 形状 → 公开 HTTP 响应 schema → SDK 重新生成 → App diff 预览与 apply 结果的展示位、文案与 en/zh/zht 三语。展示形态（在哪、怎么措辞、是否可折叠）是产品决策，且「把授权事实揭示给用户」正是 Phase F 审批中心拥有的界面。**注意实现位置的坑**：`agent-asset.ts` 目前把 `config` 当字符串原样存（`config: frontmatter.config || ""`），警示必须在 `parseAgentAssetConfig` 解码之后做。仍为「不阻断，只揭示」。它缓解的是 §3.1 attended 天花板那一行**唯一的外来资产路径**（导入别人的资产夹带 allow-all），故 attended 天花板已落地不代表本项可以取消 | TBD | **Custom M3 Phase F 开工时**（与审批中心同批，共用展示面与 i18n） |
