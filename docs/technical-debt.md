...
| ~~Location 卸载没有可测的生产路径~~ **已闭环，2026-08-27** | core | 原判断错误，来源是没读 `LayerMap` 的接口就下结论：除了 `location-layer.ts:282` 硬编码的 `idleTimeToLive: "60 minutes"` 之外，`LayerMap` 还有显式的 `invalidate(key)`。TTL 只是自动触发器，`invalidate` 是同一套拆除的手动入口，所以**不需要为了可测性把 TTL 改成可注入**。已由 `mcp-location-isolation.test.ts` 覆盖：真 `LocationServiceMap` 起一个 Location、连上 MCP 子进程、`invalidate` 后子进程被回收、重新 `get` 同一个 ref 得到干净实例。顺带测出一个事实：`LayerMap` 是引用计数的，借用未释放时 `invalidate` 只做标记、finalizer 不会跑（实测，已写进测试注释）。红证：删掉 `McpConnection.layer` 的 `addFinalizer(shutdown)` → 子进程留存 |
