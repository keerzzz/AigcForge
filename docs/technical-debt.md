...
| 负债 | 包 | 说明与根治方向 | Owner | 触发条件 |
|---|---|---|---|---|
| HttpApi exerciser 的 auth 硬门禁无有效信号 | aigcfroge | **来源：Custom M3 Phase F2 审批复核，2026-08-26。** `--mode auth --fail-on-missing --fail-on-skip` 当时为 **0 pass / 285 fail**；连公开的 `/global/health` 也统一报 `TaskDriver runtime state is not provided`。因此共同根因在 auth probe 的 runtime 装配而非 permission 路由，不能归为 F2 回归。`docs/testing.md` §3 仍把 auth 列为硬门禁，当前全红既不能验证认证边界，也不能可靠阻断回归。根治：为 `runAuth`/`callAuthProbe` 复用或补齐与正常 exerciser 相同的 TaskDriver runtime 生命周期，并恢复 public/protected 探针的有效断言。 | TBD | 修 HttpApi exerciser auth runtime 装配时；在把 auth 结果作为 merge blocker 前 |
