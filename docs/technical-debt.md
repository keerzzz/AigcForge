...
| 负债 | 包 | 说明与根治方向 | Owner | 触发条件 |
|---|---|---|---|---|
| `requestOn` 不复查 `expires`，会话中途凭据过期只能靠远端 401 现形 | core | **来源：Custom M3 Phase G 覆盖调研，2026-08-27。** `mcp/connection.ts` 的 `requestOn` 在每次带 credential 的调用前重新 `resolve` 绑定（撤销即刻生效，已有红证），但**不重新判断 OAuth `expires`**。后果：连接建立后 token 过期，本地无从得知，只能等远端返回 401，再被映射成 `CredentialMissingError` → health `auth-required`。终态对用户是对的（去授权），但**归因是错的**——「过期」与「凭据不存在」在日志和 UI 上无法区分，且依赖远端一定回 401（返回 403 或 200-with-error 的 server 会掉到 `offline`/`degraded`）。ADR-21 §4.1 第 6 条只裁定了「过期 = typed fail 进 `auth-required`、不实现 refresh」，**没有裁定过期该由哪一侧发现**，所以这不是可以由执行方顺手改的保真度问题。连接期已有 `isTokenExpired` 形状可复用（`aigcfroge/src/mcp/v2-auth.ts:139`）。根治：在 `requestOn` 复用同一判定并映射成 `CredentialExpiredError`，或明确接受远端权威并把文案改成不声称过期 | TBD | 决定过期归因由本地还是远端负责时 |
| Location 卸载没有可测的生产路径，既有清理断言全部关的是合成 Scope | core | **来源：Custom M3 Phase G，2026-08-27。** Phase G 的注入清单含「Location unload」，但生产侧没有显式卸载 API：唯一路径是 `location-layer.ts:282` 的 `LayerMap` `idleTimeToLive: "60 minutes"` 空闲驱逐，且该值硬编码、不可注入。因此**两条路都走不通**：用 `TestClock` 推进 60 分钟无法与真实 MCP 子进程共存（子进程要 `it.live`），而另建一个短 TTL 的 `LayerMap` 等于断言测试自己的装配而非生产的（这正是既有 MCP/registry/grant 清理断言的共同弱点——它们关的都是 `Scope.make()` / `Layer.fresh`，不是真的 Location）。`location-layer.test.ts` 因此无 scope-close 用例。**不写合成测试是刻意的**：伪造一个通过的卸载断言比没有更糟。根治：把 TTL 做成可注入（为可测性改生产，需裁定），或接受「运维上的回滚是重启进程」并在文档中如实写明卸载不被单测覆盖 | TBD | 决定是否为可测性开放 TTL 注入时 |
