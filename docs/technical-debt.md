...
| 负债 | 包 | 说明与根治方向 | Owner | 触发条件 |
|---|---|---|---|---|
| MCP health 表禁止 `ready → auth-required`，于是「调用期发现凭据问题」的既有意图永远不可达 | core | **来源：Custom M3 Phase G，2026-08-27（人类裁定「过期由本地判定」后实施时发现）。** 会话中途过期已修：`requestOn` 现在复用与 connect 同一个 `isCredentialExpired` 谓词，过期即 typed `CredentialExpiredError`（红证：删掉该判断后调用直接成功）。**但 health 反映不了它**：`transitionHealth` 的合法边里 `ready` 只能去 `connecting/degraded/offline/revoked`，`ready → auth-required` 被显式列为非法且有一条经过复审的测试钉住（`mcp-connection.test.ts` 的边表用例）。**这不是本次改动引入的**——`requestOn` 末尾的 `tapError` 早就把 `CredentialExpiredError`/`CredentialMissingError` 映射到 `auth-required`，但它 `.pipe(Effect.catch(() => Effect.void))` 吞掉了失败，所以从 ready 连接出发这条意图**从来没有生效过**。后果：连接不可用而 Builder 仍显示 `ready`，用户看不出要去重新授权。两条路都需要裁定：(a) 补 `ready → auth-required` 边并同步修改那条复审过的边表测试——理由是生产代码本就声明了该意图；(b) 走 `ready → connecting → auth-required` 两步（当前表下合法），代价是观察者可能捕捉到误导性的 `connecting` 中间态。倾向 (a)。**未擅自修改经过复审的状态机** |
