...
| **M3** | MCP 与审批                | scoped registration、凭证、健康、统一审批入口        | M2 + Tool Registry 扩展  | 进行中（G3-1/G3-2/G3-3/G3-4 全过；Phase A/B/D/F0 与 Phase C Slice 0-2 已合入本地 `main`；当前 Phase C Slice 3-4，余 E/F/G） |
...
当前执行手册：M0/M1/M2 已完成并合入 `main`（M2 = PR #46，合并提交 `a11b50020`，复审报告 [Custom M2 复审报告](../review/AigcForge_CUSTOM_M2_REVIEW.md) 结论 APPROVED）。**M3 进行中**（2026-08-25 状态）：四道 Gate 已全过 —— G3-1 [ADR-19](../architecture/adr/ADR-19-mcp-scoped-registration.md) Accepted v1.0、G3-2 [ADR-20](../architecture/adr/ADR-20-scoped-grant-model.md) Accepted v1.2、G3-3 [ADR-21](../architecture/adr/ADR-21-mcp-credential-custody.md) Accepted v1.2（Slice 0 独立事实复核已履行，v1.2 追加 rebind 恢复路径裁定）、G3-4 由 ADR-20 §2.6/§2.7/§2.8 回答；Phase A（`7a2804624`）/ B（`99dce8906`）/ D（`38d82e2b3`）/ F0（`229e3eb7d`）已合入本地 `main`，**Phase C Slice 0-2（typed stdio 连接 owner + `mcp_credential_binding` + 两项止血）亦已合入**。**剩余：Phase C Slice 3-4（remote/OAuth + 六态 health、断线重连与漂移）→ Phase E（Resolver/Snapshot）→ Phase F 本体（审批中心 UI）→ Phase G（故障注入与灰度）**，随后才是 M4/M5。执行提示词见 [M3 剩余阶段执行提示词](../plan/prompt-custom-mode-m3-remaining.md)（Phase C Slice 1-2 阶段的提示词见 [prompt-custom-mode-m3-mcp-approval.md](../plan/prompt-custom-mode-m3-mcp-approval.md)）。M3 全部 Phase 完成后统一开一个 PR。
