> **研究状态：BASELINE CLOSED / DECISIONS TRANSFERRED（2026-08-31）**
> 本文保留调研阶段的方案对比与待决问题。持久委派对话、多参与者、Codex 审查版本屏障、恢复和关闭语义已收敛到 [ADR-22](../../architecture/adr/ADR-22-meta-agent-persistent-delegation.md) 与 [实施计划](../../plan/meta-agent-persistent-delegation-closed-loop.md)。
...
> 后续目标（2026-08-16）：[ADR-17](../../architecture/adr/ADR-17-custom-mode-composition-platform.md) 与 [Custom PRD](../../prd/custom-mode-composition-platform.md) 提议固定第五 `custom`。本研究中的四模式描述仍是 2026-08-11 代码事实，不应被读成反对后续五值迁移。
...
| **Product Mode**              | Session 的持久分类与边界；当前代码为四值，ADR-17 提议增加固定 Custom | ADR-11；目标 ADR-17                                      | 不是智能体，是容器             | 当前 `chat/coding/work/assistant`；目标另加 `custom`           |
...
| `chat-orchestrator`            | primary  | `* deny` + 只读 + propose\_\*\_asset                      | Chat 专属（委派目标）                       |
| `work-orchestrator`            | primary  | `* deny` + 只读 + work-preset + task\_\* 增量             | Work 专属（委派目标）                       |
...
| 4   | assistant 模式缺执行者                                                   | **需新建 `assistant` 子智能体**：fail-closed 权限承载个人事项（reminder/kb*\*/propose_note/memory*\*），meta 在 assistant 场景委派给它                               |
