...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/aigcfroge|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/aigcfroge` or `/oc`")
  }
}
...
async function assertAigcfrogeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to aigcfroge server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.aigcfroge.ai" : "https://aigcfroge.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.aigcfroge.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("aigcfroge-github-action")
    response = await fetch("https://api.aigcfroge.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/aigcfroge" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/aigcfroge") || body.includes("/oc")) {
      if (reviewContext) {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `aigcfroge/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/aigcfroge-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[aigcfroge session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
