...
brew install anomalyco/tap/aigcfroge # macOS 및 Linux (권장, 항상 최신)
brew install aigcfroge              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S aigcfroge            # Arch Linux (Stable)
paru -S aigcfroge-bin               # Arch Linux (Latest from AUR)
mise use -g aigcfroge               # 어떤 OS든
nix run nixpkgs#aigcfroge           # 또는 github:keerzzz/AigcForge 로 최신 dev 브랜치
```
...
Aigcfroge 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
Aigcfroge 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### Aigcfroge 기반으로 만들기

Aigcfroge 와 관련된 프로젝트를 진행하면서 이름에 "aigcfroge"(예: "aigcfroge-dashboard" 또는 "aigcfroge-mobile") 를 포함한다면, README 에 해당 프로젝트가 Aigcfroge 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/aigcfroge) | [X.com](https://x.com/aigcfroge)
