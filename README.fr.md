...
brew install anomalyco/tap/aigcfroge # macOS et Linux (recommandé, toujours à jour)
brew install aigcfroge              # macOS et Linux (formule officielle brew, mise à jour moins fréquente)
sudo pacman -S aigcfroge            # Arch Linux (Stable)
paru -S aigcfroge-bin               # Arch Linux (Latest from AUR)
mise use -g aigcfroge               # n'importe quel OS
nix run nixpkgs#aigcfroge           # ou github:keerzzz/AigcForge pour la branche dev la plus récente
```
...
1. `$AIGCFROGE_INSTALL_DIR` - Répertoire d'installation personnalisé
2. `$XDG_BIN_DIR` - Chemin conforme à la spécification XDG Base Directory
3. `$HOME/bin` - Répertoire binaire utilisateur standard (s'il existe ou peut être créé)
4. `$HOME/.aigcfroge/bin` - Repli par défaut
...
Aigcfroge inclut deux agents intégrés que vous pouvez basculer avec la touche `Tab`.
...
Si vous souhaitez contribuer à Aigcfroge, lisez nos [docs de contribution](./CONTRIBUTING.md) avant de soumettre une pull request.

### Construire avec Aigcfroge

Si vous travaillez sur un projet lié à Aigcfroge et que vous utilisez "aigcfroge" dans le nom du projet (par exemple, "aigcfroge-dashboard" ou "aigcfroge-mobile"), ajoutez une note dans votre README pour préciser qu'il n'est pas construit par l'équipe Aigcfroge et qu'il n'est pas affilié à nous.

---

**Rejoignez notre communauté** [Discord](https://discord.gg/aigcfroge) | [X.com](https://x.com/aigcfroge)
