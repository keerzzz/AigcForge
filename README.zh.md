...
- **核心能力**：
  ...
  - **三条资产供给路径**：
    1. _引导创建（Guided Creation）_：通过结构化问答推断资产类型与参数并生成草稿。
    2. _会话捕获（Session Capture）_：在任意模式的消息气泡上一键“存为资产”，预填并转换。
    3. _外部导入（External Import）_：支持粘贴外部 AI 对话或导入文件，自动过滤思考过程与对话噪声，提取纯净资产。
...
2. 在默认 **Coding 模式** 下，输入：
   > _"重构 packages/core 中的 session 缓存逻辑，并利用 @explore 检查所有依赖方的调用方式。"_
...
2. 输入提示词创建指令，例如：
   > _"帮我制作一个前端 Code Review 的技能，要求重点检查 Effect 错误处理和 Tailwind Token V2 的使用规范。"_
...
> _"@claude-code 请利用官方 SDK 深入排查底层 node-pty 进程挂起的问题并输出解决方案。"_
