...
import { getAigcfrogeModeStack, AIGCFROGE_BASE_MODE, AigcfrogeKeymapProvider, registerAigcfrogeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAigcfrogeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when aigcfroge mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAigcfrogeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAigcfrogeModeStack(keymap).push("autocomplete")
