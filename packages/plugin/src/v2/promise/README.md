# Aigcfroge V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@aigcfroge/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an Aigcfroge extension point.
...
```ts
import { define } from "@aigcfroge/plugin/v2/promise"
