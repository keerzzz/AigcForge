---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Aigcfroge.
---

import config from "../../../../config.mjs"
export const console = config.console

Aigcfroge bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Aigcfroge-konfigurasjonen.
...
i `~/.local/share/aigcfroge/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Aigcfroge
konfig.
...
Aigcfroge Zen er en liste over modeller levert av Aigcfroge-teamet som har vært
testet og verifisert for å fungere godt med Aigcfroge. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Aigcfroge Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Aigcfroge Zen` og gå til [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Det fungerer som alle andre leverandører i Aigcfroge og er helt valgfritt å bruke.

---

## Aigcfroge Go

Aigcfroge Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Aigcfroge-teamet som har vært
testet og verifisert for å fungere godt med Aigcfroge.

1. Kjør kommandoen `/connect` i TUI, velg `Aigcfroge Go`, og gå til [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Slik bruker du Amazon Bedrock med Aigcfroge:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Aigcfroge:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `aigcfroge.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i aigcfroge støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere aigcfroge til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Aigcfroge skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Aigcfroge-konfigurasjonen.
...
for å lagre token i Aigcfroge auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Aigcfroge bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Aigcfroge
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`aigcfroge.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `aigcfroge.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Aigcfroge:
...
Slik bruker du Google Vertex AI med Aigcfroge:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Aigcfroge.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Aigcfroge, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) plugin for automatisk å logge hver Aigcfroge-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Aigcfroge-samtale oppført som en separat økt.
...
Du kan konfigurere Aigcfroge for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Aigcfroge for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Aigcfroge for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Aigcfroge. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/aigcfroge) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Aigcfroge:
...
3. Kopier API-nøkkelen for bruk i Aigcfroge.
...
6. **Viktig**: Før du bruker skymodeller i Aigcfroge, må du hente modellinformasjonen lokalt:
...
Aigcfroge Zen er en liste over testede og verifiserte modeller levert av Aigcfroge-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Aigcfroge Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Aigcfroge Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Aigcfroge API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Aigcfroge-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Aigcfroge-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Aigcfroge:
...
Du kan også tilpasse modeller gjennom Aigcfroge-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Aigcfroge. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `aigcfroge.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Aigcfroge forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `aigcfroge auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Aigcfroge-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Aigcfroge-konfigurasjonen.
