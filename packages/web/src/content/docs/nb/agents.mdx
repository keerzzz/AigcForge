...
Det er to typer agenter i Aigcfroge; primære agenter og underagenter.
...
Aigcfroge kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Aigcfroge kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Aigcfroge kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `aigcfroge.json`:
...
Hvis ingen temperatur er spesifisert, bruker Aigcfroge modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Aigcfroge-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Aigcfroge-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Aigcfroge Zen](/docs/zen), vil du bruke `aigcfroge/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `aigcfroge models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/keerzzz/AigcForge).
:::
