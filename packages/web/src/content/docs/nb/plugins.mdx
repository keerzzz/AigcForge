...
description: Skriv dine egne programtillegg for å utvide Aigcfroge.
---

Plugins lar deg utvide Aigcfroge ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Aigcfroge.
...
- `.aigcfroge/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/aigcfroge/node_modules/`.
...
1. Global konfigurasjon (`~/.config/aigcfroge/aigcfroge.json`)
2. Prosjektkonfigurasjon (`aigcfroge.json`)
3. Global plugin-katalog (`~/.config/aigcfroge/plugins/`)
4. Prosjektpluginkatalog (`.aigcfroge/plugins/`)
...
Aigcfroge kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Aigcfroge SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Aigcfroge.
...
:::note
Hvis du bruker Aigcfroge-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Aigcfroge fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Aigcfroge:
...
`tool`-hjelperen lager et tilpasset verktøy som Aigcfroge kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Aigcfroge sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://aigcfroge.ai/docs/sdk) for detaljer.
