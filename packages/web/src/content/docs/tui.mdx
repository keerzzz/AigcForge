---
title: TUI
description: Using the Aigcfroge terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Aigcfroge provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Aigcfroge starts the TUI for the current directory.
...
When using the Aigcfroge TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Aigcfroge. Allows you to select from available providers and add their API keys.
...
Exit Aigcfroge. _Aliases_: `/quit`, `/q`
...
This is separate from `aigcfroge.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Aigcfroge waits after the leader key. Defaults to `2000`.
...
Use `AIGCFROGE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `aigcfroge.default`.
