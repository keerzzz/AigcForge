---
title: サーバー
description: HTTP 経由で Aigcfroge サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aigcfroge serve` コマンドは、Aigcfroge クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `aigcfroge.local` |
...
HTTP 基本認証でサーバーを保護するには、`AIGCFROGE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `aigcfroge` になるか、`AIGCFROGE_SERVER_USERNAME` を設定してオーバーライドします。これは、`aigcfroge serve` と `aigcfroge web` の両方に当てはまります。
...
`aigcfroge` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Aigcfroge サーバーを使用して、プログラムで Aigcfroge と対話します。
:::

`aigcfroge serve` を実行してスタンドアロンサーバーを起動できます。すでに Aigcfroge TUI を実行している場合でも、`aigcfroge serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Aigcfroge [IDE](/docs/ide) プラグインによって使用されます。
