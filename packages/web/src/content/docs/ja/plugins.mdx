---
title: プラグイン
description: Aigcfroge を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Aigcfroge を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Aigcfroge のデフォルトの動作を変更したりできます。
...
- `.aigcfroge/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/aigcfroge/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/aigcfroge/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/aigcfroge/aigcfroge.json`)
2. プロジェクト設定 (`aigcfroge.json`)
3. グローバルプラグインディレクトリ (`~/.config/aigcfroge/plugins/`)
4. プロジェクトプラグインディレクトリ (`.aigcfroge/plugins/`)
...
Aigcfroge は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Aigcfroge SDK クライアント。
...
ここでは、Aigcfroge を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://aigcfroge.ai/docs/sdk) を参照してください。
...
Aigcfroge が `.env` ファイルを読み取らないようにします。
...
プラグインは Aigcfroge にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Aigcfroge が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Aigcfroge で使用できます。
