...
Создайте `.aigcfroge/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию aigcfroge или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем aigcfroge [config](/docs/config):
...
- Глобальный: `~/.config/aigcfroge/commands/`
- Для каждого проекта: `.aigcfroge/commands/`
...
aigcfroge включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
