---
title: Server
description: Interact with aigcfroge server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `aigcfroge serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an aigcfroge client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `aigcfroge.local` |
...
Set `AIGCFROGE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `aigcfroge`, or set `AIGCFROGE_SERVER_USERNAME` to override it. This applies to both `aigcfroge serve` and `aigcfroge web`.
...
When you run `aigcfroge` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the aigcfroge server to interact with aigcfroge programmatically.
:::

This architecture lets aigcfroge support multiple clients and allows you to interact with aigcfroge programmatically.

You can run `aigcfroge serve` to start a standalone server. If you have the
aigcfroge TUI running, `aigcfroge serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Aigcfroge [IDE](/docs/ide) plugins.
...
The aigcfroge server exposes the following APIs.
