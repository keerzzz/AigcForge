...
For at fejlfinde problemer med Aigcfroge, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\aigcfroge\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `aigcfroge --log-level DEBUG`.

---

## Lagring

aigcfroge gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/aigcfroge/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\aigcfroge`
...
Aigcfroge Desktop kører en lokal Aigcfroge-server (`aigcfroge-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Aigcfroge`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/aigcfroge/aigcfroge.jsonc` (eller `~/.config/aigcfroge/aigcfroge.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/aigcfroge/aigcfroge.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\aigcfroge\aigcfroge.jsonc`
...
Aigcfroge kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/aigcfroge/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\aigcfroge\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Aigcfroge kan genopbygge den.

1. Afslut Aigcfroge Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/aigcfroge`
- **Linux**: slet `~/.cache/aigcfroge` (eller kør `rm -rf ~/.cache/aigcfroge`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\aigcfroge`

3. Start Aigcfroge Desktop på ny.

---

### Løs problemer med serverforbindelse

Aigcfroge Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `aigcfroge.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `AIGCFROGE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `AIGCFROGE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Aigcfroge Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Aigcfroge.

---

### Meddelelser vises ikke

Aigcfroge Desktop viser kun systemvarsler når:

- varsler er aktiveret for Aigcfroge i dine OS-indstillinger, og
...
1. Afslut Aigcfroge Desktop.
2. Find og slet disse filer (de findes i Aigcfroge Desktop-appens datamappe):

- `aigcfroge.settings.dat` (skrivebordsstandardserver URL)
- `aigcfroge.global.dat` og `aigcfroge.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Aigcfroge:
...
For at finde ud af hvilke modeller du har adgang til, kør `aigcfroge models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\aigcfroge`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. aigcfroge installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\aigcfroge`

2. Start aigcfroge på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge aigcfroge til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
aigcfroge vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
