...
Med Aigcfroge kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Aigcfroge vårt eget `aigcfroge`-tema.
...
Aigcfroge kommer med flere innebygde temaer.
...
- Vil at Aigcfroge skal matche terminalens utseende
...
Aigcfroge støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/aigcfroge/themes/*.json` eller `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.aigcfroge/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.aigcfroge/themes/*.json`
