...
Her er alle de innebygde verktøyene som er tilgængelige i Aigcfroge.
...
:::note
Dette verktøyet er bare tilgængelig når `AIGCFROGE_EXPERIMENTAL_LSP_TOOL=true` (eller `AIGCFROGE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Aigcfroge-leverandøren eller når miljøvariabelen `AIGCFROGE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
