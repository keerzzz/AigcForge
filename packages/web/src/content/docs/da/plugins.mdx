---
title: Plugins
description: Skriv dine egne plugins for at udvide Aigcfroge.
---

Plugins giver dig mulighed for at udvide Aigcfroge ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Aigcfroges standardadfærd.
...
- `.aigcfroge/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/aigcfroge/node_modules/`.
...
1. Global konfiguration (`~/.config/aigcfroge/aigcfroge.json`)
2. Projektkonfiguration (`aigcfroge.json`)
3. Global plugin-mappe (`~/.config/aigcfroge/plugins/`)
4. Projekt plugin bibliotek (`.aigcfroge/plugins/`)
...
Aigcfroge kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En aigcfroge SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide aigcfroge.
...
:::note
Hvis du bruger Aigcfroge desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå aigcfroge i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til aigcfroge:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som aigcfroge kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for aigcfroge sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://aigcfroge.ai/docs/sdk) for detaljer.
