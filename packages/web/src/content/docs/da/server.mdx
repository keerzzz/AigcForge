---
title: Server
description: Interager med aigcfroge-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `aigcfroge serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en aigcfroge-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `aigcfroge.local` |
...
Indstil `AIGCFROGE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `aigcfroge`, eller indstil `AIGCFROGE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `aigcfroge serve` og `aigcfroge web`.
...
Når du kører `aigcfroge` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug aigcfroge-serveren til at interagere med aigcfroge programmatisk.
:::

Denne arkitektur lader aigcfroge understøtte flere klienter og lader dig interagere med aigcfroge programmatisk.

Du kan køre `aigcfroge serve` for at starte en selvstændig server. Hvis du har
aigcfroge TUI kørende, vil `aigcfroge serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Aigcfroge [IDE](/docs/ide) plugins.
...
Aigcfroge-serveren viser følgende API'er.
