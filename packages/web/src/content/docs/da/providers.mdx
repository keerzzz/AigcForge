---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Aigcfroge.
---

import config from "../../../../config.mjs"
export const console = config.console

Aigcfroge bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Aigcfroge-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/aigcfroge/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Aigcfroge-konfigurationen.
...
Aigcfroge Zen er en liste over modeller leveret af Aigcfroge-teamet, som er blevet testet og verificeret til at fungere godt med Aigcfroge. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Aigcfroge Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg aigcfroge og gå til [aigcfroge.ai/auth](https://aigcfroge.ai/auth).
...
Det fungerer som alle andre udbydere i Aigcfroge og er helt valgfrit at bruge.

---

## Aigcfroge Go

Aigcfroge Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Aigcfroge-teamet, som er testet og verificeret til at fungere godt med Aigcfroge.

1. Kør kommandoen `/connect` i TUI, vælg `Aigcfroge Go`, og gå til [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Sådan bruger du Amazon Bedrock med Aigcfroge:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører aigcfroge:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `aigcfroge.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Aigcfroge understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere aigcfroge til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at aigcfroge skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem aigcfroge-konfigurationen.
...
gemme tokenet i aigcfroge auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Aigcfroge bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Aigcfroge
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`aigcfroge.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `aigcfroge.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Aigcfroge:
...
Sådan bruger du Google Vertex AI med Aigcfroge:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører aigcfroge.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem aigcfroge, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) plugin for automatisk at logge hver Aigcfroge-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Aigcfroge-samtale opført som en separat session.
...
Du kan konfigurere aigcfroge til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere aigcfroge til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere aigcfroge til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Aigcfroge. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/aigcfroge) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Aigcfroge:
...
3. Kopier API-nøglen til brug i Aigcfroge.
...
6. **Vigtigt**: Før du bruger skymodeller i Aigcfroge, skal du hente modelinformationen lokalt:
...
Aigcfroge Zen er en liste over testede og verificerede modeller leveret af Aigcfroge-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Aigcfroge Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Aigcfroge Zen**.

   ```txt
   /connect
   ```

3. Indtast Aigcfroge API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem aigcfroge-konfigurationen.
...
5. Du kan også tilpasse dem gennem aigcfroge-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Aigcfroge:
...
Du kan også tilpasse modeller gennem aigcfroge-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med aigcfroge. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `aigcfroge.json`-filen i projektkataloget:
...
`limit`-felterne lader Aigcfroge forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `aigcfroge auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek aigcfroge-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i aigcfroge-konfigurationen.
