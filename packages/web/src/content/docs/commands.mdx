...
Create `.aigcfroge/commands/test.md`:
...
You can add custom commands through the Aigcfroge config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Aigcfroge [config](/docs/config):
...
- Per-project: `.aigcfroge/commands/`
...
aigcfroge includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
