...
Mit Aigcfroge kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Aigcfroge das `aigcfroge`-Theme.
...
Aigcfroge liefert mehrere eingebaute Themes mit.
...
- Aigcfroge optisch ans Terminal anpassen wollen
...
Aigcfroge unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/aigcfroge/themes/*.json` oder `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.aigcfroge/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.aigcfroge/themes/*.json`
