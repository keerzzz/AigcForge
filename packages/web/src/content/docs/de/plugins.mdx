---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Aigcfroge zu erweitern.
---

Mit Plugins können Sie Aigcfroge erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Aigcfroge zu ändern.
...
- `.aigcfroge/plugins/` – Plugins auf Projektebene
- `~/.config/aigcfroge/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/aigcfroge/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/aigcfroge/aigcfroge.json`)
2. Projektkonfiguration (`aigcfroge.json`)
3. Globales Plugin-Verzeichnis (`~/.config/aigcfroge/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.aigcfroge/plugins/`)
...
Aigcfroge führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Aigcfroge-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Aigcfroge erweitern können.
...
:::note
Wenn Sie die Aigcfroge-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Aigcfroge `.env`-Dateien liest:
...
Plugins können Aigcfroge auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Aigcfroge aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Aigcfroge zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://aigcfroge.ai/docs/sdk).
