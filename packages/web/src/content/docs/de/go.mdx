...
Aigcfroge Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Aigcfroge. Du abonnierst Aigcfroge Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Aigcfroge zu verwenden.
...
Aigcfroge Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Aigcfroge Go funktioniert wie jeder andere Provider in Aigcfroge.

1. Du meldest dich bei **<a href={console}>Aigcfroge Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Aigcfroge Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Aigcfroge Go abonnieren.
:::
...
Aigcfroge Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Aigcfroge Config verwendet das Format `aigcfroge-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `aigcfroge-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Aigcfroge Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Aigcfroge zu nutzen.
