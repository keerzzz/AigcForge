...
Agent Skills erlauben Aigcfroge, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Aigcfroge sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Aigcfroge vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.aigcfroge/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.aigcfroge/skills/git-release/SKILL.md` zum Beispiel so:
...
Aigcfroge listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `aigcfroge.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `aigcfroge.json`):
