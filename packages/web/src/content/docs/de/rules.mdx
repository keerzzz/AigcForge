---
title: Regeln
description: Lege eigene Anweisungen fuer aigcfroge fest.
---

Du kannst aigcfroge ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in aigcfroge den Befehl `/init` aus.
...
So kann aigcfroge sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht aigcfroge sie zu erweitern.
...
aigcfroge kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/aigcfroge/AGENTS.md` speichern.
Diese gelten in allen aigcfroge-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Aigcfroge dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/aigcfroge/AGENTS.md` existiert)
...
Beim Start sucht aigcfroge in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/aigcfroge/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `aigcfroge.json` oder in `~/.config/aigcfroge/aigcfroge.json` angeben.
...
aigcfroge parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit aigcfroge.json

Empfohlen ist das `instructions`-Feld in der `aigcfroge.json`:
...
Du kannst aigcfroge explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `aigcfroge.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
