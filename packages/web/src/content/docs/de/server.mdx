---
title: Server
description: Nutze den aigcfroge-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `aigcfroge serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein aigcfroge-Client nutzen kann.
...
Setze `AIGCFROGE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `aigcfroge`, kann aber mit `AIGCFROGE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `aigcfroge serve` und `aigcfroge web`.
...
Wenn du `aigcfroge` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den aigcfroge-Server, um aigcfroge programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `aigcfroge serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `aigcfroge serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Aigcfroge-[IDE](/docs/ide)-Plugins.
...
Der aigcfroge-Server stellt folgende APIs bereit.
