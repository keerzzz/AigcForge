...
Ajan becerileri, aigcfroge'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
aigcfroge su konumlari tarar:

- Proje konfigurasyonu: `.aigcfroge/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Proje yerel yollarinda aigcfroge, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.aigcfroge/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.aigcfroge/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
aigcfroge, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`aigcfroge.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`aigcfroge.json` icinde):
