---
title: Sunucu
description: aigcfroge sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aigcfroge serve` komutu, aigcfroge istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `aigcfroge.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `AIGCFROGE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `aigcfroge` değeridir; değiştirmek isterseniz `AIGCFROGE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `aigcfroge serve` hem de `aigcfroge web` için geçerlidir.
...
`aigcfroge` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
aigcfroge ile programatik etkileşim için aigcfroge sunucusunu kullanın.
:::

Bu mimari, aigcfroge'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`aigcfroge serve` ile bağımsız bir sunucu başlatabilirsiniz. aigcfroge TUI açıksa, `aigcfroge serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum aigcfroge [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
aigcfroge sunucusu aşağıdaki API'leri sunar.
