...
aigcfroge ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak aigcfroge kendi `aigcfroge` temasını kullanır.
...
aigcfroge birden fazla yerleşik temayla gelir.
...
- aigcfroge'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
aigcfroge, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/aigcfroge/themes/*.json` veya `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.aigcfroge/themes/*.json`
4. **Geçerli çalışma dizini** - `./.aigcfroge/themes/*.json`
