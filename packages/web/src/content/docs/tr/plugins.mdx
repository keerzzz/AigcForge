---
title: Eklentiler
description: Aigcfroge'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Aigcfroge'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Aigcfroge'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.aigcfroge/plugins/` - Proje düzeyinde eklentiler
- `~/.config/aigcfroge/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/aigcfroge/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/aigcfroge/aigcfroge.json`)
2. Project config (`aigcfroge.json`)
3. Global eklenti dizini (`~/.config/aigcfroge/plugins/`)
4. Proje eklenti dizini (`.aigcfroge/plugins/`)
...
Aigcfroge bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Aigcfroge SDK istemcisi.
...
Aigcfroge'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Aigcfroge masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Aigcfroge'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Aigcfroge'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Aigcfroge'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Aigcfroge'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://aigcfroge.ai/docs/sdk) bakın.
