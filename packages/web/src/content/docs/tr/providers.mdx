---
title: Sağlayıcılar
description: aigcfroge'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

aigcfroge, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı aigcfroge yapılandırmanızda yapılandırın.
...
`~/.local/share/aigcfroge/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları aigcfroge'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Aigcfroge Zen, aigcfroge ekibi tarafından sağlanan modellerin bir listesidir.
aigcfroge ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Aigcfroge Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, aigcfroge'u seçin ve [aigcfroge.ai/auth](https://aigcfroge.ai/auth)'ye gidin.
...
aigcfroge'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Aigcfroge Go

Aigcfroge Go, aigcfroge ile iyi çalıştığı test edilmiş ve doğrulanmış, aigcfroge ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Aigcfroge Go`'yu seçin ve [aigcfroge.ai/auth](https://aigcfroge.ai/zen) adresine gidin.
...
Amazon Bedrock'u aigcfroge ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   aigcfroge'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `aigcfroge.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi aigcfroge'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
aigcfroge'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   aigcfroge'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Aigcfroge yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
aigcfroge, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. aigcfroge'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`aigcfroge.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth) ana sayfasında.

##### Yapılandırma

`aigcfroge.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi aigcfroge'la kullanmak için:
...
Google Vertex AI'yi aigcfroge ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları aigcfroge çalıştırılırken ayarlayın.
...
Helicone'un aigcfroge aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her aigcfroge görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her aigcfroge görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
aigcfroge'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla aigcfroge'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla aigcfroge'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini aigcfroge için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/aigcfroge)'a bakın.
:::
...
Ollama Cloud'u aigcfroge ile kullanmak için:
...
3. aigcfroge'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: aigcfroge'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Aigcfroge Zen, aigcfroge ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Aigcfroge Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Aigcfroge Zen**'i arayın.

   ```txt
   /connect
   ```

3. aigcfroge API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   aigcfroge yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca aigcfroge yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Aigcfroge ile kullanmak için:
...
Modelleri aigcfroge yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı aigcfroge ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `aigcfroge.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları aigcfroge'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `aigcfroge auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için aigcfroge yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, aigcfroge yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
