...
Aigcfroge Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Aigcfroge'daki diğer sağlayıcılar gibi çalışır. Aigcfroge Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Aigcfroge'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Aigcfroge Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Aigcfroge Go, Aigcfroge'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Aigcfroge Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Aigcfroge Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Aigcfroge Go'ya abone olabilir.
:::
...
Aigcfroge Go aşağıdaki limitleri içerir:
...
Aigcfroge yapılandırmanızdaki [model id](/docs/config/#models) formatı `aigcfroge-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `aigcfroge-go/kimi-k2.7-code` kullanmalısınız.
...
Aigcfroge Go'yu şu amaçlarla oluşturduk:
...
4. Aigcfroge ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
