...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. aigcfroge yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
aigcfroge içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `AIGCFROGE_EXPERIMENTAL_LSP_TOOL=true` (veya `AIGCFROGE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca aigcfroge provider kullanılırken veya `AIGCFROGE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

aigcfroge başlatırken etkinleştirmek için:
