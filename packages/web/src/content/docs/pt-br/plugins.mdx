---
title: Plugins
description: Escreva seus próprios plugins para estender o aigcfroge.
---

Plugins permitem que você estenda o aigcfroge conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do aigcfroge.
...
- `.aigcfroge/plugins/` - Plugins em nível de projeto
- `~/.config/aigcfroge/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/aigcfroge/node_modules/`.
...
1. Configuração global (`~/.config/aigcfroge/aigcfroge.json`)
2. Configuração do projeto (`aigcfroge.json`)
3. Diretório de plugins global (`~/.config/aigcfroge/plugins/`)
4. Diretório de plugins do projeto (`.aigcfroge/plugins/`)
...
O aigcfroge executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do aigcfroge para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o aigcfroge.
...
:::note
Se você estiver usando o aplicativo desktop aigcfroge, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o aigcfroge de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao aigcfroge:
...
O helper `tool` cria uma ferramenta personalizada que o aigcfroge pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o aigcfroge junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://aigcfroge.ai/docs/sdk) para detalhes.
