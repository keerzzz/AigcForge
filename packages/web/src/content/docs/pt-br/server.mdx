---
title: Servidor
description: Interaja com o servidor aigcfroge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `aigcfroge serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente aigcfroge pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `aigcfroge.local` |
...
Defina `AIGCFROGE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `aigcfroge`, ou defina `AIGCFROGE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `aigcfroge serve` quanto ao `aigcfroge web`.
...
Quando você executa `aigcfroge`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor aigcfroge para interagir com o aigcfroge programaticamente.
:::

Essa arquitetura permite que o aigcfroge suporte múltiplos clientes e permite que você interaja com o aigcfroge programaticamente.

Você pode executar `aigcfroge serve` para iniciar um servidor autônomo. Se você tiver o TUI do aigcfroge em execução, `aigcfroge serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do aigcfroge [IDE](/docs/ide).
...
O servidor aigcfroge expõe as seguintes APIs.
