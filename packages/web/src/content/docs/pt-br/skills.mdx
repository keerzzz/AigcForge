...
As habilidades do agente permitem que o aigcfroge descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O aigcfroge pesquisa nesses locais:

- Configuração do projeto: `.aigcfroge/skills/<name>/SKILL.md`
- Configuração global: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o aigcfroge sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.aigcfroge/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.aigcfroge/skills/git-release/SKILL.md` assim:
...
O aigcfroge lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `aigcfroge.json`:
...
**Para agentes embutidos** (em `aigcfroge.json`):
