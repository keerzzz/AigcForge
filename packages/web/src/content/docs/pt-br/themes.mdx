...
Com o aigcfroge, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o aigcfroge usa nosso próprio tema `aigcfroge`.
...
O aigcfroge vem com vários temas embutidos.
...
- Querem que o aigcfroge corresponda à aparência do seu terminal
...
O aigcfroge suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/aigcfroge/themes/*.json` ou `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.aigcfroge/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.aigcfroge/themes/*.json`
