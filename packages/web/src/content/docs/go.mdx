...
Aigcfroge Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Aigcfroge. You subscribe to Aigcfroge Go and
...
Aigcfroge Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Aigcfroge Go works like any other provider in Aigcfroge.

1. You sign in to **<a href={console}>Aigcfroge Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Aigcfroge Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Aigcfroge Go.
:::
...
Aigcfroge Go includes the following limits:
...
uses the format `aigcfroge-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `aigcfroge-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Aigcfroge as well.
