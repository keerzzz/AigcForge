...
Puoi aggiungere strumenti esterni a Aigcfroge usando il _Model Context Protocol_, o MCP. Aigcfroge supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Aigcfroge Config](https://aigcfroge.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/aigcfroge`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Aigcfroge gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Aigcfroge:
...
Se il server richiede autenticazione, Aigcfroge ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `aigcfroge mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Aigcfroge memorizzera' i token in modo sicuro in `~/.local/share/aigcfroge/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Aigcfroge insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Aigcfroge come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Aigcfroge al tuo account Sentry.
