...
Con Aigcfroge puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Aigcfroge usa il tema `aigcfroge`.
...
Aigcfroge include diversi temi integrati.
...
- Vuole che Aigcfroge corrisponda all'aspetto del terminale
...
Aigcfroge supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/aigcfroge/themes/*.json` o `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.aigcfroge/themes/*.json`
4. **Directory di lavoro corrente** - in `./.aigcfroge/themes/*.json`
