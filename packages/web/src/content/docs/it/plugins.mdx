---
title: Plugin
description: Scrivi plugin per estendere Aigcfroge.
---

I plugin ti permettono di estendere Aigcfroge agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Aigcfroge.
...
- `.aigcfroge/plugins/` - plugin a livello progetto
- `~/.config/aigcfroge/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/aigcfroge/node_modules/`.
...
1. Config globale (`~/.config/aigcfroge/aigcfroge.json`)
2. Config di progetto (`aigcfroge.json`)
3. Directory plugin globale (`~/.config/aigcfroge/plugins/`)
4. Directory plugin di progetto (`.aigcfroge/plugins/`)
...
Aigcfroge esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di aigcfroge per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere aigcfroge.
...
:::note
Se usi l'app desktop di Aigcfroge, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a aigcfroge di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a aigcfroge:
...
L'helper `tool` crea uno strumento personalizzato che aigcfroge può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in aigcfroge insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://aigcfroge.ai/docs/sdk) per i dettagli.
