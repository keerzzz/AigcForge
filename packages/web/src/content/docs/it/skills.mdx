...
Le skill degli agenti permettono a Aigcfroge di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Aigcfroge cerca in queste posizioni:

- Config di progetto: `.aigcfroge/skills/<name>/SKILL.md`
- Config globale: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Aigcfroge risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.aigcfroge/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.aigcfroge/skills/git-release/SKILL.md` cosi':
...
Aigcfroge elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `aigcfroge.json`:
...
**Per agenti integrati** (in `aigcfroge.json`):
