---
title: Server
description: Interagisci con il server aigcfroge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `aigcfroge serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client aigcfroge.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `aigcfroge.local` |
...
Imposta `AIGCFROGE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `aigcfroge`, oppure imposta `AIGCFROGE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `aigcfroge serve` sia per `aigcfroge web`.
...
Quando esegui `aigcfroge` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server aigcfroge per interagire con aigcfroge in modo programmatico.
:::

Questa architettura permette a aigcfroge di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `aigcfroge serve` per avviare un server standalone. Se la TUI di aigcfroge e gia in esecuzione, `aigcfroge serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Aigcfroge.
...
Il server aigcfroge espone le seguenti API.
