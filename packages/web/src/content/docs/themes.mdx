...
With Aigcfroge you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Aigcfroge uses our own `aigcfroge` theme.
...
Aigcfroge comes with several built-in themes.
...
- Want Aigcfroge to match their terminal's appearance
...
Aigcfroge supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/aigcfroge/themes/*.json` or `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.aigcfroge/themes/*.json`
4. **Current working directory** - Defined in `./.aigcfroge/themes/*.json`
