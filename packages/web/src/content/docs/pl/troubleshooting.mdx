...
Aby debugować problemy z aigcfroge, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aigcfroge\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `aigcfroge --log-level DEBUG`.

---

## Przechowywanie danych

aigcfroge przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/aigcfroge/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aigcfroge`
...
aigcfroge Desktop uruchamia w tle lokalny serwer aigcfroge (sidecar `aigcfroge-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Aigcfroge` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/aigcfroge/aigcfroge.jsonc` (lub `~/.config/aigcfroge/aigcfroge.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/aigcfroge/aigcfroge.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aigcfroge\aigcfroge.jsonc`
...
aigcfroge może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/aigcfroge/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aigcfroge\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby aigcfroge mógł ją odbudować.

1. Całkowicie zamknij aigcfroge Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/aigcfroge`
- **Linux**: usuń `~/.cache/aigcfroge` (lub uruchom `rm -rf ~/.cache/aigcfroge`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\aigcfroge`

3. Uruchom ponownie aigcfroge Desktop.

---

### Napraw problemy z połączeniem z serwerem

aigcfroge Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `aigcfroge.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `AIGCFROGE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `AIGCFROGE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows aigcfroge Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami aigcfroge.

---

### Brak powiadomień

aigcfroge Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla aigcfroge w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie aigcfroge Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji aigcfroge Desktop):

- `aigcfroge.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `aigcfroge.global.dat` i `aigcfroge.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z aigcfroge:
...
3. Upewnij się, że masz najnowszą wersję z `aigcfroge upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `aigcfroge models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\aigcfroge`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. aigcfroge dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\aigcfroge`

2. Uruchom ponownie kod aigcfroge, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to aigcfroge do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
aigcfroge wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
