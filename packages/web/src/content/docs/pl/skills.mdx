...
Umiejętności agenta pozwalają aigcfroge odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
aigcfroge przeszukuje te lokalizacje:

- Project config: `.aigcfroge/skills/<name>/SKILL.md`
- Global config: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu aigcfroge przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.aigcfroge/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.aigcfroge/skills/git-release/SKILL.md` w ten sposób:
...
aigcfroge wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `aigcfroge.json`:
...
**Dla agentów wbudowanych** (w `aigcfroge.json`):
