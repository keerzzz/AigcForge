---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w aigcfroge.
---

import config from "../../../../config.mjs"
export const console = config.console

aigcfroge używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji aigcfroge.
...
w `~/.local/share/aigcfroge/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim aigcfroge
config.
...
Aigcfroge Zen to lista modeli dostarczonych przez zespół aigcfroge, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z aigcfroge. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Aigcfroge Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Aigcfroge Zen` i przejdź do [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Działa jak każdy inny dostawca w aigcfroge i jest całkowicie opcjonalny w użyciu.

---

## Aigcfroge Go

Aigcfroge Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół aigcfroge, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z aigcfroge.

1. Uruchom polecenie `/connect` w TUI, wybierz `Aigcfroge Go` i przejdź do [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Aby używać Amazon Bedrock z aigcfroge:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania aigcfroge:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `aigcfroge.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w aigcfroge nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować aigcfroge do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod aigcfroge działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji aigcfroge.
...
aigcfroge używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować aigcfroge
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`aigcfroge.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem aigcfroge:
...
Aby używać Google Vertex AI z aigcfroge:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania aigcfroge.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą aigcfroge, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session), aby automatycznie rejestrować każdą konwersację aigcfroge jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja aigcfroge będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować aigcfroge tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować aigcfroge tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować aigcfroge tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem aigcfroge. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/aigcfroge).
:::
...
Aby korzystać z Ollama Cloud z aigcfroge:
...
3. Skopiuj klucz API do użycia w aigcfroge.
...
6. **Ważne**: Przed użyciem modeli chmurowych w aigcfroge musisz pobrać informacje o modelu lokalnie:
...
Aigcfroge Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół aigcfroge. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Aigcfroge Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Aigcfroge Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API aigcfroge.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji aigcfroge.
...
5. Możesz je także dostosować za pomocą konfiguracji aigcfroge. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z aigcfroge:
...
Możesz także dostosować modele za pomocą konfiguracji aigcfroge. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z aigcfroge. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `aigcfroge.json` w katalogu projektu:
...
Pola `limit` pozwalają aigcfroge zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `aigcfroge auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację aigcfroge i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji aigcfroge.
