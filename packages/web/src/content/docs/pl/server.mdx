---
title: Serwer
description: Interakcja z serwerem aigcfroge za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `aigcfroge serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient aigcfroge.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `aigcfroge.local` |
...
Ustaw `AIGCFROGE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `aigcfroge` lub ustaw `AIGCFROGE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `aigcfroge serve`, jak i `aigcfroge web`.
...
Kiedy uruchomisz `aigcfroge`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera aigcfroge do programowej interakcji z aigcfroge.
:::

Ta architektura umożliwia obsługę wielu klientów przez aigcfroge i programową interakcję z kodem otwartym.

Możesz uruchomić `aigcfroge serve`, aby uruchomić samodzielny serwer. Jeśli masz
aigcfroge TUI działa, `aigcfroge serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki aigcfroge [IDE](/docs/ide).
...
Serwer aigcfroge udostępnia następujące interfejsy API.
