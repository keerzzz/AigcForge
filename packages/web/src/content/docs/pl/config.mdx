---
title: Konfiguracja
description: Ustawienia Aigcfroge w pliku JSON.
---

Możesz dostosować Aigcfroge za pomocą pliku konfiguracyjnego JSON.

---

## Format

Aigcfroge obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "aigcfroge"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/aigcfroge`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/aigcfroge/aigcfroge.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `AIGCFROGE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`aigcfroge.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.aigcfroge`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `AIGCFROGE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.aigcfroge` i `~/.config/aigcfroge` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/aigcfroge`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Aigcfroge w `~/.config/aigcfroge/aigcfroge.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/aigcfroge/tui.json`.
...
Dodaj `aigcfroge.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Aigcfroge się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `AIGCFROGE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `AIGCFROGE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.aigcfroge` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.aigcfroge`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`aigcfroge.ai/config.json`**](https://aigcfroge.ai/config.json).
...
Użyj `AIGCFROGE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `aigcfroge.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `aigcfroge serve` i `aigcfroge web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Aigcfroge.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `aigcfroge.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Aigcfroge wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/aigcfroge/agents/` lub `.aigcfroge/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Aigcfroge powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`aigcfroge run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/aigcfroge/commands/` lub `.aigcfroge/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Aigcfroge może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Aigcfroge nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Aigcfroge **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Aigcfroge o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.aigcfroge/plugins/` lub `~/.config/aigcfroge/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Aigcfroge tylko do określonych dostawców.
