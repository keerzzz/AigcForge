...
Możesz dodać dodatkowe informacje do aigcfroge za pomocą _Model Context Protocol_, w skrócie MCP. aigcfroge obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [aigcfroge Config](https://aigcfroge.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/aigcfroge`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
aigcfroge automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, aigcfroge:
...
Jeśli serwer wymaga uwierzytelnienia, aigcfroge poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `aigcfroge mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji aigcfroge bezpieczne przechowa tokeny w `~/.local/share/aigcfroge/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w aigcfroge, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie aigcfroge, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć aigcfroge z kontem Sentry.
