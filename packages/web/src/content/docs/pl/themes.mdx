...
Dzięki aigcfroge możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie aigcfroge używa naszego własnego motywu `aigcfroge`.
...
aigcfroge ma kilka wbudowanych motywów.
...
- Chcą, aby Aigcfroge pasował do wyglądu ich terminala
...
aigcfroge obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/aigcfroge/themes/*.json` lub `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.aigcfroge/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.aigcfroge/themes/*.json`
