---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć aigcfroge.
---

Wtyczki umożliwiające wyłączenie aigcfroge poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania aigcfroge.
...
- `.aigcfroge/plugins/` - Wtyczki na poziomie projektu
- `~/.config/aigcfroge/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/aigcfroge/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/aigcfroge/aigcfroge.json`)
2. Konfiguracja projektu (`aigcfroge.json`)
3. Globalny katalog wtyczek (`~/.config/aigcfroge/plugins/`)
4. Katalog wtyczek projektu (`.aigcfroge/plugins/`)
...
aigcfroge uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z aigcfroge do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do aigcfroge.
...
:::note
Jeśli korzystasz z aplikacji komputerowej aigcfroge, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj aigcfroge czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do aigcfroge:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod aigcfroge. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://aigcfroge.ai/docs/sdk).
