...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Aigcfroge posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Aigcfroge.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `AIGCFROGE_EXPERIMENTAL_LSP_TOOL=true` (lub `AIGCFROGE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Aigcfroge lub gdy zmienna środowiskowa `AIGCFROGE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Aigcfroge:
