...
يمكنك إضافة أدوات خارجية إلى Aigcfroge باستخدام _Model Context Protocol_، أو MCP. يدعم Aigcfroge خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Aigcfroge](https://aigcfroge.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/aigcfroge` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Aigcfroge تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Aigcfroge بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Aigcfroge المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `aigcfroge mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Aigcfroge الرموز بشكل آمن في `~/.local/share/aigcfroge/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Aigcfroge إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Aigcfroge مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Aigcfroge بحساب Sentry الخاص بك.
