...
أنشئ الملف `.aigcfroge/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Aigcfroge أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Aigcfroge:
...
- على مستوى النظام: `~/.config/aigcfroge/commands/`
- لكل مشروع: `.aigcfroge/commands/`
...
يتضمن aigcfroge عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
