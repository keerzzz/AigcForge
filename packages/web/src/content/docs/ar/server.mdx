---
title: الخادم
description: تفاعل مع خادم aigcfroge عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `aigcfroge serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل aigcfroge استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `aigcfroge.local` |
...
عيّن `AIGCFROGE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `aigcfroge`، أو عيّن `AIGCFROGE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `aigcfroge serve` و `aigcfroge web`.
...
عند تشغيل `aigcfroge` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم aigcfroge للتفاعل مع aigcfroge برمجيا.
:::

تتيح هذه البنية لـ aigcfroge دعم عدة عملاء وتمكّنك من التفاعل مع aigcfroge برمجيا.

يمكنك تشغيل `aigcfroge serve` لبدء خادم مستقل. إذا كانت واجهة aigcfroge في terminal (TUI)
قيد التشغيل، فسيبدأ `aigcfroge serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Aigcfroge لـ [IDE](/docs/ide).
...
يعرض خادم aigcfroge واجهات API التالية.
