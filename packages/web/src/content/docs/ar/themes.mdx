...
مع Aigcfroge يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Aigcfroge سمتنا `aigcfroge`.
...
يأتي Aigcfroge مع عدة سمات مدمجة.
...
- يريدون أن يطابق Aigcfroge مظهر terminal لديهم
...
يدعم Aigcfroge نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/aigcfroge/themes/*.json` أو `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.aigcfroge/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.aigcfroge/themes/*.json`
