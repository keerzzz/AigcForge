---
title: المزوّدون
description: استخدام أي موفّر LLM في Aigcfroge.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Aigcfroge ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Aigcfroge.
...
في `~/.local/share/aigcfroge/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Aigcfroge.
...
Aigcfroge Zen هي قائمة نماذج يوفّرها فريق Aigcfroge وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Aigcfroge. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Aigcfroge Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر aigcfroge، ثم انتقل إلى [aigcfroge.ai/auth](https://aigcfroge.ai/auth).
...
يعمل مثل أي مزوّد آخر في Aigcfroge واستخدامه اختياري بالكامل.

---

## Aigcfroge Go

Aigcfroge Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Aigcfroge، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Aigcfroge.

1. شغّل الأمر `/connect` في TUI، واختر `Aigcfroge Go`، ثم انتقل إلى [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
لاستخدام Amazon Bedrock مع Aigcfroge:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل aigcfroge:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `aigcfroge.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Aigcfroge غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين aigcfroge لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل aigcfroge بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات aigcfroge.
...
تخزين الرمز في مساحة تخزين مصادقة aigcfroge.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Aigcfroge نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Aigcfroge
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`aigcfroge.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth).

##### التهيئة

خصّص عبر `aigcfroge.json`:
...
لاستخدام اشتراك GitHub Copilot مع aigcfroge:
...
لاستخدام Google Vertex AI مع Aigcfroge:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر aigcfroge، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) لتسجيل كل محادثة Aigcfroge تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Aigcfroge مدرجة كجلسة مستقلة.
...
يمكنك ضبط aigcfroge لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط aigcfroge لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط aigcfroge لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Aigcfroge. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/aigcfroge) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Aigcfroge:
...
3. انسخ مفتاح API لاستخدامه في Aigcfroge.
...
6. **مهم**: قبل استخدام نماذج السحابة في Aigcfroge، يجب سحب معلومات النموذج محليا:
...
Aigcfroge Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Aigcfroge. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Aigcfroge Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Aigcfroge Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Aigcfroge.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات aigcfroge.
...
5. يمكنك أيضا تخصيصها عبر إعدادات aigcfroge. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Aigcfroge:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات aigcfroge. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع aigcfroge. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `aigcfroge.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Aigcfroge بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `aigcfroge auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات aigcfroge وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات aigcfroge.
