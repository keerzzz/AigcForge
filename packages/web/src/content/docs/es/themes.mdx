...
Con Aigcfroge puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Aigcfroge usa nuestro propio tema `aigcfroge`.
...
Aigcfroge viene con varios temas integrados.
...
- Quiere que Aigcfroge coincida con la apariencia de su terminal
...
Aigcfroge admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/aigcfroge/themes/*.json` o `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.aigcfroge/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.aigcfroge/themes/*.json`
