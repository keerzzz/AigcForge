---
title: Complementos
description: Escriba sus propios complementos para extender Aigcfroge.
---

Los complementos le permiten extender Aigcfroge al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Aigcfroge.
...
- `.aigcfroge/plugins/` - Complementos a nivel de proyecto
- `~/.config/aigcfroge/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/aigcfroge/node_modules/`.
...
1. Configuración global (`~/.config/aigcfroge/aigcfroge.json`)
2. Configuración del proyecto (`aigcfroge.json`)
3. Directorio global de complementos (`~/.config/aigcfroge/plugins/`)
4. Directorio de complementos del proyecto (`.aigcfroge/plugins/`)
...
Aigcfroge ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK aigcfroge para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar aigcfroge.
...
:::note
Si está utilizando la aplicación de escritorio Aigcfroge, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que aigcfroge lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a aigcfroge:
...
El ayudante `tool` crea una herramienta personalizada a la que aigcfroge puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para aigcfroge junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://aigcfroge.ai/docs/sdk) para obtener más detalles.
