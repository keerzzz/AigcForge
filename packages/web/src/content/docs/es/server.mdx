---
title: Servidor
description: Interactuar con el servidor aigcfroge a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `aigcfroge serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente aigcfroge puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `aigcfroge.local` |
...
Configure `AIGCFROGE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `aigcfroge`, o configure `AIGCFROGE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `aigcfroge serve` como a `aigcfroge web`.
...
Cuando ejecuta `aigcfroge`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor aigcfroge para interactuar con aigcfroge mediante programación.
:::

Esta arquitectura permite que aigcfroge admita múltiples clientes y le permite interactuar con aigcfroge mediante programación.

Puede ejecutar `aigcfroge serve` para iniciar un servidor independiente. Si tienes el
aigcfroge TUI ejecutándose, `aigcfroge serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Aigcfroge [IDE](/docs/ide).
...
El servidor aigcfroge expone las siguientes API.
