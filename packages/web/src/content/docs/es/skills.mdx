...
Las habilidades del agente permiten a Aigcfroge descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Aigcfroge busca estas ubicaciones:

- Configuración del proyecto: `.aigcfroge/skills/<name>/SKILL.md`
- Configuración global: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Aigcfroge sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.aigcfroge/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.aigcfroge/skills/git-release/SKILL.md` así:
...
Aigcfroge enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `aigcfroge.json`:
...
**Para agentes integrados** (en `aigcfroge.json`):
