...
You can add external tools to Aigcfroge using the _Model Context Protocol_, or MCP. Aigcfroge supports both local and remote servers.
...
You can define MCP servers in your [Aigcfroge Config](https://aigcfroge.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/aigcfroge` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Aigcfroge automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Aigcfroge will:
...
If the server requires authentication, Aigcfroge will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `aigcfroge mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Aigcfroge will store the tokens securely in `~/.local/share/aigcfroge/mcp-auth.json`.
...
Your MCPs are available as tools in Aigcfroge, alongside built-in tools. So you can manage them through the Aigcfroge config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Aigcfroge to your Sentry account.
