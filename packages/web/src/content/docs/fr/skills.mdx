...
Les compétences d'agent permettent à Aigcfroge de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Aigcfroge recherche ces emplacements :

- Configuration du projet : `.aigcfroge/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Aigcfroge remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.aigcfroge/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.aigcfroge/skills/git-release/SKILL.md` comme ceci :
...
Aigcfroge répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `aigcfroge.json` :
...
**Pour les agents intégrés** (dans `aigcfroge.json`) :
