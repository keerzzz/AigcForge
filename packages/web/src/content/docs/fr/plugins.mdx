---
title: Plugins
description: Écrivez vos propres plugins pour étendre Aigcfroge.
---

Les plugins vous permettent d'étendre Aigcfroge en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Aigcfroge.
...
- `.aigcfroge/plugins/` - Plugins au niveau du projet
- `~/.config/aigcfroge/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/aigcfroge/node_modules/`.
...
1. Configuration globale (`~/.config/aigcfroge/aigcfroge.json`)
2. Configuration du projet (`aigcfroge.json`)
3. Répertoire global des plugins (`~/.config/aigcfroge/plugins/`)
4. Répertoire des plugins du projet (`.aigcfroge/plugins/`)
...
Aigcfroge exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK aigcfroge pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre aigcfroge.
...
:::note
Si vous utilisez l'application de bureau Aigcfroge, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez aigcfroge de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à aigcfroge :
...
L'assistant `tool` crée un outil personnalisé que aigcfroge peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour aigcfroge aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://aigcfroge.ai/docs/sdk) pour plus de détails.
