...
Avec Aigcfroge, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Aigcfroge utilise notre propre thème `aigcfroge`.
...
Aigcfroge est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Aigcfroge corresponde à l'apparence de leur terminal
...
Aigcfroge prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/aigcfroge/themes/*.json` ou `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.aigcfroge/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.aigcfroge/themes/*.json`
