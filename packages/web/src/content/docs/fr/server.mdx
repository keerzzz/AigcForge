---
title: Serveur
description: Interagissez avec le serveur aigcfroge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `aigcfroge serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client aigcfroge peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `aigcfroge.local` |
...
Définissez `AIGCFROGE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `aigcfroge`, ou définissez `AIGCFROGE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `aigcfroge serve` et à `aigcfroge web`.
...
Lorsque vous exécutez `aigcfroge`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur aigcfroge pour interagir avec aigcfroge par programmation.
:::

Cette architecture permet à aigcfroge de prendre en charge plusieurs clients et vous permet d'interagir avec aigcfroge par programme.

Vous pouvez exécuter `aigcfroge serve` pour démarrer un serveur autonome. Si vous avez le
aigcfroge TUI en cours d'exécution, `aigcfroge serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Aigcfroge [IDE](/docs/ide).
...
Le serveur aigcfroge expose les API suivantes.
