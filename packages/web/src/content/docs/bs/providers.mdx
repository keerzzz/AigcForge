---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Aigcfroge.
---

import config from "../../../../config.mjs"
export const console = config.console

Aigcfroge koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Aigcfroge konfiguraciji.
...
u `~/.local/share/aigcfroge/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Aigcfroge
config.
...
Aigcfroge Zen je lista modela koje je obezbedio Aigcfroge tim koji su bili
testirano i potvrđeno da dobro radi sa Aigcfroge. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Aigcfroge Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite aigcfroge i idite na [aigcfroge.ai/auth](https://aigcfroge.ai/auth).
...
Radi kao i svaki drugi provajder u Aigcfroge i potpuno je opcionalan za korištenje.

---

## Aigcfroge Go

Aigcfroge Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Aigcfroge tim i koji su testirani i verificirani da dobro rade s Aigcfroge-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Aigcfroge Go` i idite na [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
Da biste koristili Amazon Bedrock s Aigcfroge:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete aigcfroge:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `aigcfroge.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Aigcfroge.
:::
...
Možete konfigurirati aigcfroge za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi aigcfroge ispravno radio.
   :::
...
Također možete dodati modele kroz svoju aigcfroge konfiguraciju.
...
da pohrani token u aigcfroge auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Aigcfroge koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Aigcfroge
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`aigcfroge.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth).

##### Konfiguracija

Prilagodite putem `aigcfroge.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s aigcfroge:
...
Za korištenje Google Vertex AI s Aigcfroge:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće aigcfroge.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem aigcfrogea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) da automatski prijavite svaki Aigcfroge razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Aigcfroge razgovor naveden kao zasebna sesija.
...
Možete konfigurirati aigcfroge za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati aigcfroge za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati aigcfroge za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Aigcfroge. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/aigcfroge) za detalje.
:::
...
Da biste koristili Ollama Cloud s Aigcfroge:
...
3. Kopirajte API ključ za korištenje u Aigcfroge.
...
6. **Važno**: Prije upotrebe modela oblaka u Aigcfroge, morate lokalno povući informacije o modelu:
...
Aigcfroge Zen je lista testiranih i verifikovanih modela koju je obezbedio Aigcfroge tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Aigcfroge Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Aigcfroge Zen**.

```txt
   /connect
```

3. Unesite svoj Aigcfroge API ključ.
...
Također možete dodati dodatne modele putem vaše aigcfroge konfiguracije.
...
5. Također ih možete prilagoditi putem vaše aigcfroge konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Aigcfrogeom:
...
Također možete prilagoditi modele kroz svoju aigcfroge konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s aigcfroge-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `aigcfroge.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Aigcfroge da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `aigcfroge auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Aigcfroge konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj aigcfroge konfiguraciji.
