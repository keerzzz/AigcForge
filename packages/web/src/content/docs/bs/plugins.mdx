---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Aigcfroge.
---

Dodaci vam omogućavaju da proširite Aigcfroge spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Aigcfroge.
...
- `.aigcfroge/plugins/` - Dodaci na nivou projekta
- `~/.config/aigcfroge/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/aigcfroge/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/aigcfroge/aigcfroge.json`)
2. Konfiguracija projekta (`aigcfroge.json`)
3. Globalni direktorij dodataka (`~/.config/aigcfroge/plugins/`)
4. Direktorij dodataka projekta (`.aigcfroge/plugins/`)
...
Aigcfroge pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Aigcfroge SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Aigcfroge.
...
Spriječite aigcfroge da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u aigcfroge:
...
Pomoćnik `tool` kreira prilagođeni alat koji aigcfroge može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za aigcfroge zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://aigcfroge.ai/docs/sdk) za detalje.
