...
U Aigcfroge mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Aigcfroge koristi nasu `aigcfroge` temu.
...
Aigcfroge dolazi sa vise ugradenih tema.
...
- Zele da Aigcfroge odgovara izgledu njihovog terminala
...
Aigcfroge podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/aigcfroge/themes/*.json` ili `$XDG_CONFIG_HOME/aigcfroge/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.aigcfroge/themes/*.json`
4. **Trenutni radni direktorij** - `./.aigcfroge/themes/*.json`
