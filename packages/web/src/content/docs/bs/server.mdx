---
title: Server
description: Komunicirajte s aigcfroge serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `aigcfroge serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji aigcfroge klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `aigcfroge.local` |
...
Postavite `AIGCFROGE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `aigcfroge`, ili postavite `AIGCFROGE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `aigcfroge serve` i za `aigcfroge web`.
...
Kada pokrenete `aigcfroge`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite aigcfroge server za programsku interakciju sa aigcfroge-om.
:::

Ova arhitektura omogucava aigcfroge podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `aigcfroge serve` da startate standalone server. Ako je aigcfroge TUI vec pokrenut, `aigcfroge serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Aigcfroge [IDE](/docs/ide) pluginovi.
...
aigcfroge server izlaže sljedece API-je.
