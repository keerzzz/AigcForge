...
Agent skills omogucavaju Aigcfroge da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Aigcfroge pretrazuje ove lokacije:

- Konfiguracija projekta: `.aigcfroge/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/aigcfroge/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Aigcfroge ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.aigcfroge/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/aigcfroge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.aigcfroge/skills/git-release/SKILL.md` ovako:
...
Aigcfroge navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `aigcfroge.json`:
...
**Za ugradene agente** (u `aigcfroge.json`):
