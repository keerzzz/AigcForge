...
Da biste otklonili probleme s Aigcfroge, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aigcfroge\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `aigcfroge --log-level DEBUG`.

---

## Pohrana

Aigcfroge pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/aigcfroge/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aigcfroge`
...
Aigcfroge Desktop pokreće lokalni Aigcfroge server (`aigcfroge-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Aigcfroge` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/aigcfroge/aigcfroge.jsonc` (ili `~/.config/aigcfroge/aigcfroge.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/aigcfroge/aigcfroge.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aigcfroge\aigcfroge.jsonc`
...
Aigcfroge također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/aigcfroge/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aigcfroge\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Aigcfroge mogao ponovo izgraditi.

1. Potpuno zatvorite Aigcfroge Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/aigcfroge`
- **Linux**: obrišite `~/.cache/aigcfroge` (ili pokrenite `rm -rf ~/.cache/aigcfroge`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\aigcfroge`

3. Ponovo pokrenite Aigcfroge Desktop.

---

### Rješavanje problema sa vezom na serveru

Aigcfroge Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `aigcfroge.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `AIGCFROGE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `AIGCFROGE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Aigcfroge Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Aigcfroge karakteristikama.

---

### Obavještenja se ne prikazuju

Aigcfroge Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Aigcfroge u postavkama vašeg OS-a, i
...
1. Zatvorite Aigcfroge Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Aigcfroge Desktop aplikacije):

- `aigcfroge.settings.dat` (URL zadanog servera za desktop)
- `aigcfroge.global.dat` i `aigcfroge.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Aigcfroge:
...
Da saznate kojim modelima imate pristup, pokrenite `aigcfroge models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\aigcfroge`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Aigcfroge dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\aigcfroge`

2. Ponovo pokrenite Aigcfroge da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Aigcfroge da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Aigcfroge će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
