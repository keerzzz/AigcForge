...
Možete dodati vanjske alate u Aigcfroge koristeći _Model Context Protocol_, ili MCP. Aigcfroge podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Aigcfroge Config](https://aigcfroge.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/aigcfroge`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Aigcfroge automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Aigcfroge će:
...
Ako server zahtijeva autentifikaciju, Aigcfroge će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `aigcfroge mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Aigcfroge sigurno čuva tokene u `~/.local/share/aigcfroge/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Aigcfroge, zajedno s ugrađenim alatima. Možete njima upravljati kroz Aigcfroge konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Aigcfroge sa vašim Sentry nalogom.
