...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Aigcfroge dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Aigcfroge.
...
:::note
Ovaj alat je dostupan samo kada je `AIGCFROGE_EXPERIMENTAL_LSP_TOOL=true` (ili `AIGCFROGE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Aigcfroge provajdera ili kada je varijabla `AIGCFROGE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Aigcfroge:
