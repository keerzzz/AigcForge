---
title: Konfiguracija
description: Korištenje Aigcfroge JSON konfiguracije.
---

Možete konfigurirati Aigcfroge koristeći JSON konfiguracijski fajl.

---

## Format

Aigcfroge podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/aigcfroge`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/aigcfroge/aigcfroge.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`AIGCFROGE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`aigcfroge.json` u projektu) - postavke specifične za projekat
5. **`.aigcfroge` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`AIGCFROGE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.aigcfroge` i `~/.config/aigcfroge` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/aigcfroge` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Aigcfroge konfiguraciju u `~/.config/aigcfroge/aigcfroge.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/aigcfroge/tui.json`.
...
Dodajte `aigcfroge.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Aigcfroge pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `AIGCFROGE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `AIGCFROGE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.aigcfroge` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.aigcfroge`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`aigcfroge.ai/config.json`**](https://aigcfroge.ai/config.json).

TUI konfiguracija koristi [**`aigcfroge.ai/tui.json`**](https://aigcfroge.ai/tui.json).
...
Koristite `AIGCFROGE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `aigcfroge.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `aigcfroge serve` i `aigcfroge web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Aigcfroge server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `aigcfroge.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Aigcfroge konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Aigcfroge pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/aigcfroge/agents/` ili `.aigcfroge/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Aigcfroge će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`aigcfroge run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/aigcfroge/commands/` ili `.aigcfroge/commands/`. [Saznajte više ovdje](/docs/commands).
...
Aigcfroge će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Aigcfroge **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Aigcfroge sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.aigcfroge/plugins/` ili `~/.config/aigcfroge/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Aigcfroge da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
