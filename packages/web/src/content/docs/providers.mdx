---
title: Providers
description: Using any LLM provider in Aigcfroge.
---

import config from "../../../config.mjs"
export const console = config.console

Aigcfroge uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Aigcfroge config.
...
in `~/.local/share/aigcfroge/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Aigcfroge
config.
...
Aigcfroge Zen is a list of models provided by the Aigcfroge team that have been
tested and verified to work well with Aigcfroge. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Aigcfroge Zen.
:::

1. Run the `/connect` command in the TUI, select `Aigcfroge Zen`, and head to [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
It works like any other provider in Aigcfroge and is completely optional to use.

---

## Aigcfroge Go

Aigcfroge Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Aigcfroge team that have been
tested and verified to work well with Aigcfroge.

1. Run the `/connect` command in the TUI, select `Aigcfroge Go`, and head to [aigcfroge.ai/auth](https://aigcfroge.ai/zen).
...
To use Amazon Bedrock with Aigcfroge:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running aigcfroge:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `aigcfroge.json`:
...
Aigcfroge. Anthropic explicitly prohibits this.

Previous versions of Aigcfroge came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Aigcfroge with zero setup:
...
You can configure aigcfroge to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for aigcfroge to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your aigcfroge config.
...
Aigcfroge supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Aigcfroge uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Aigcfroge. Sign in and approve.

   :::note
   Aigcfroge requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Aigcfroge surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Aigcfroge forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Aigcfroge is experimental. Features, configuration, and
behavior may change in future releases.
:::

Aigcfroge integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in aigcfroge auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Aigcfroge uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Aigcfroge
to only use your own GitLab-hosted instance, add the following to your
`aigcfroge.json` file. It is also recommended to disable session sharing.
...
More documentation on [aigcfroge-gitlab-auth](https://www.npmjs.com/package/aigcfroge-gitlab-auth) homepage.

##### Configuration

Customize through `aigcfroge.json`:
...
When a `duo-workflow-*` model is selected, Aigcfroge will:
...
4. Route tool execution requests through Aigcfroge's permission-gated tool system
...
To use your GitHub Copilot subscription with aigcfroge:
...
To use Google Vertex AI with Aigcfroge:
...
2. Set the required environment variables:
   ...
   Set them while running aigcfroge.
...
In the event you see a feature or model from Helicone that isn't configured automatically through aigcfroge, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [aigcfroge-helicone-session](https://github.com/H2Shami/aigcfroge-helicone-session) plugin to automatically log each Aigcfroge conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Aigcfroge conversation listed as a separate session.
...
You can configure aigcfroge to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure aigcfroge to use local models through LM Studio.
...
You can configure aigcfroge to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Aigcfroge. See the [Ollama integration docs](https://docs.ollama.com/integrations/aigcfroge) for details.
:::
...
To use Ollama Cloud with Aigcfroge:
...
3. Copy the API key for use in Aigcfroge.
...
6. **Important**: Before using cloud models in Aigcfroge, you must pull the model information locally:
...
Aigcfroge Zen is a list of tested and verified models provided by the Aigcfroge team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Aigcfroge Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Aigcfroge Zen**.

   ```txt
   /connect
   ```

3. Enter your Aigcfroge API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your aigcfroge config.
...
5. You can also customize them through your aigcfroge config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Aigcfroge:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=aigcfroge&utm_medium=docs&utm_campaign=cortex-provider).

aigcfroge's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Aigcfroge captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting aigcfroge:
...
The model catalog is provided automatically. A minimal `aigcfroge.json` is all that's needed:
...
You can also customize models through your aigcfroge config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Aigcfroge opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Aigcfroge refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Aigcfroge is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Aigcfroge — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Aigcfroge prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Aigcfroge polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with aigcfroge. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `aigcfroge.json` file in your project directory:
...
The `limit` fields allow Aigcfroge to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `aigcfroge auth list` to see if the credentials
...
2. For custom providers, check the aigcfroge config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your aigcfroge config.
