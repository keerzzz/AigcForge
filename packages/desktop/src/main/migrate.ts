...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.aigcfroge.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `aigcfroge.settings.dat` is special: it maps to the `aigcfroge.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"aigcfroge.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // aigcfroge.settings.dat → the electron settings store ("aigcfroge.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "aigcfroge.global.dat").
  const storeName = filename === "aigcfroge.settings.dat" ? "aigcfroge.settings" : filename
