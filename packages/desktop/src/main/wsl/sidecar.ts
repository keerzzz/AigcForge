...
import { type WslCommandLine, resolveWslAigcfroge, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const aigcfroge = await resolveWslAigcfroge(distro)
  if (!aigcfroge) throw new Error(`Aigcfroge is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "aigcfroge"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export AIGCFROGE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AIGCFROGE_CLIENT=desktop",
    `export AIGCFROGE_SERVER_USERNAME=${shellEscape(username)}`,
    `export AIGCFROGE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(aigcfroge)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
