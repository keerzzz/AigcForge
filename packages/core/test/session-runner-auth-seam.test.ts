...
import * as AuthSeam from "@aigcfroge/core/session/runner/auth-seam"
...
afterEach(() => {
  AuthSeam.register(() => Effect.succeed(undefined))
})
...
const runCredential = (providerID: string) =>
  AuthSeam.getCredential(providerID) as unknown as Effect.Effect<Credential.Value | undefined>
...
test("auth seam returns credential from registered resolver", async () => {
  ...
  AuthSeam.register((providerID) => Effect.succeed(providerID === "openai" ? key : undefined))
...
test("auth seam resolver inherits caller Effect context", async () => {
  ...
  const result = await Effect.runPromise(
    (AuthSeam.getCredential("openai") as unknown as Effect.Effect<
