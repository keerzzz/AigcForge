...
import { DeepInfraPlugin } from "@aigcfroge/core/plugin/provider/deepinfra"
  ...
  createDeepInfra: (options: Record<string, unknown>) => {
    ...
    return {
      languageModel: (modelID: string) => {
        ...
        return { modelID, provider: `${String(captured.name ?? "deepinfra")}.chat`, specificationVersion: "v3" }
      },
    }
  },
}))
