...
describe("MetaAgentService", () => {
  it.effect("create returns a meta agent with the given properties", () =>
    Effect.gen(function* () {
      ...
      expect(created.agent).toBe("meta")
      expect(created.model.id).toBe("gpt-4")
      expect(created.model.providerID).toBe("openai")
  ...
  it.effect("attach/detach works with real sessions", () =>
    Effect.gen(function* () {
      ...
      const pid = "proj_attach_test"
      const sid = SessionSchema.ID.make("ses_attach_sess")
      yield* db.insert(ProjectTable).values({
        id: pid, worktree: "/tmp", sandboxes: [], time_created: 1, time_updated: 1,
      })
      yield* db.insert(SessionTable).values({
        id: sid, project_id: pid, directory: "/tmp", title: "t", slug: sid, version: "0",
  ...
  it.effect("stats returns counts for attached sessions", () =>
    Effect.gen(function* () {
      ...
      const pid = "proj_stats_test"
      yield* db.insert(ProjectTable).values({
        id: pid, worktree: "/tmp", sandboxes: [], time_created: 1, time_updated: 1,
      })
      const sid1 = SessionSchema.ID.make("ses_stats_s1")
      const sid2 = SessionSchema.ID.make("ses_stats_s2")
      yield* db.insert(SessionTable).values({ id: sid1, project_id: pid, directory: "/t", title: "t", slug: sid1, version: "0", time_created: 1, time_updated: 1 })
      yield* db.insert(SessionTable).values({ id: sid2, project_id: pid, directory: "/t", title: "t", slug: sid2, version: "0", time_created: 1, time_updated: 1 })
