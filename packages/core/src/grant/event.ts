...
export class CommitRejected extends Schema.TaggedErrorClass<CommitRejected>()(
  "GrantEvent.CommitRejected",
  { grantID: Schema.String, revision: Schema.Int },
) {
      ...
      commit: (seq) => {
        ...
        return commit().pipe(
          Effect.flatMap((accepted) =>
            accepted ? Effect.void : Effect.die(new CommitRejected({ grantID: update.grantID, revision: update.revision })),
          ),
        )
      },
    })
