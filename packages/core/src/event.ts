...
export class Service extends Context.Service<Service, Interface>()("@aigcfroge/Event") {}
    ...
    Effect.gen(function* () {
      ...
      const project = <D extends Definition>(definition: D, projector: Projector<D>): Effect.Effect<void> =>
        Effect.sync(() => {
          ...
          list.push((event, tx) =>
            // oxlint-disable-next-line typescript-eslint/no-unsafe-type-assertion -- existential registry entry is paired with this projector at registration time
