export * as WorkflowAssetPath from "./path"
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(WORKFLOWS_DIR, `${normalized}.yaml`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, WORKFLOWS_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    ...
    const resource = path.posix.join(WORKFLOWS_DIR, validated)
    ...
    if (target.externalDirectory || canonicalResource !== resource) {
      return yield* new PathValidationError({ reason: "Canonical path escapes workflow asset root", path: relativePath })
    }
    return target
  })
}
