...
import type { IntegrationOAuthMethodRegistration } from "@aigcfroge/plugin/v2/effect/integration"
import { define } from "@aigcfroge/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.aigcfroge.ai"
const clientID = "aigcfroge-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Aigcfroge provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("aigcfroge", (integration) => {
        integration.name = "Aigcfroge"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "aigcfroge", method: { type: "key", label: "API key (service account)" } })
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("aigcfroge")
      ...
      const item = catalog.provider.get(ProviderV2.ID.aigcfroge)
      if (!item) return
      const hasKey = Boolean(process.env.AIGCFROGE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("aigcfroge")),
