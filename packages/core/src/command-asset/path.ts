export * as CommandAssetPath from "./path"
...
export class PathValidationError extends Schema.TaggedErrorClass<PathValidationError>()("CommandAsset.PathValidation", {
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(COMMANDS_DIR, `${normalized}.md`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, COMMANDS_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    ...
    const resource = path.posix.join(COMMANDS_DIR, validated)
    ...
    if (target.externalDirectory || canonicalResource !== resource) {
      return yield* new PathValidationError({ reason: "Canonical path escapes command asset root", path: relativePath })
    }
    return target
  })
}
