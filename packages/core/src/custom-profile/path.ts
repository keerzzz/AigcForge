export * as CustomProfilePath from "./path"
...
export function validateRelativePath(relativePath: string): string {
  ...
  if (!normalized.endsWith(".yaml")) {
    throw new PathValidationError({ reason: "Path must end with .yaml", path: relativePath })
  }
...
export function normalizeRelativePath(relativePath: string): string {
  let raw = relativePath.replaceAll("\\", "/").trim()
  const prefix = `${CUSTOM_PROFILES_DIR}/`
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(CUSTOM_PROFILES_DIR, `${normalized}.yaml`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, CUSTOM_PROFILES_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    const validated = yield* Effect.try({
      try: () => normalizeRelativePath(relativePath),
      catch: (error) =>
        error instanceof PathValidationError
          ? error
          : new PathValidationError({ reason: String(error), path: relativePath }),
    })
    const resource = path.posix.join(CUSTOM_PROFILES_DIR, validated)
