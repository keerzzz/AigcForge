export * as SkillAssetPath from "./path"
...
export class PathValidationError extends Schema.TaggedErrorClass<PathValidationError>()("SkillAsset.PathValidation", {
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(SKILLS_DIR, `${normalized}.md`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, SKILLS_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    ...
    const resource = path.posix.join(SKILLS_DIR, validated)
    ...
    if (target.externalDirectory || canonicalResource !== resource) {
      return yield* new PathValidationError({ reason: "Canonical path escapes skill asset root", path: relativePath })
    }
    return target
  })
}
