export * as AgentAssetPath from "./path"
...
export class PathValidationError extends Schema.TaggedErrorClass<PathValidationError>()("AgentAsset.PathValidation", {
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(AGENTS_DIR, `${normalized}.md`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, AGENTS_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    ...
    const resource = path.posix.join(AGENTS_DIR, validated)
    ...
    if (target.externalDirectory || canonicalResource !== resource) {
      return yield* new PathValidationError({ reason: "Canonical path escapes agent asset root", path: relativePath })
    }
    return target
  })
}
