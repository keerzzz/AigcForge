export * as MCPAssetPath from "./path"
...
export class PathValidationError extends Schema.TaggedErrorClass<PathValidationError>()("MCPAsset.PathValidation", {
...
export function nameToRelativePath(name: string): string {
  ...
  return path.posix.join(MCPS_DIR, `${normalized}.md`)
}

export function resolveOwnerRoot(locationDirectory: string): string {
  return path.resolve(locationDirectory, MCPS_DIR)
}
...
export function resolveSafeTarget(
  ...
  return Effect.gen(function* () {
    ...
    const resource = path.posix.join(MCPS_DIR, validated)
    ...
    if (target.externalDirectory || canonicalResource !== resource) {
      return yield* new PathValidationError({ reason: "Canonical path escapes mcp asset root", path: relativePath })
    }
    return target
  })
}
