...
export default {
  ...
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(
        `INSERT INTO \`__new_external_cli_session\`(\`id\`, \`session_id\`, \`participant_id\`, \`cli_target\`, \`external_session_id\`, \`status\`, \`time_created\`, \`time_updated\`) SELECT COALESCE(\`id\`, printf('ecs_%016x', rowid)), \`session_id\`, \`participant_id\`, \`cli_target\`, \`external_session_id\`, \`status\`, \`time_created\`, \`time_updated\` FROM \`external_cli_session\`;`,
      )
