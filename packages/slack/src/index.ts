...
import { createAigcfroge, type ToolPart } from "@aigcfroge/sdk"
...
console.log("🚀 Starting aigcfroge server...")
const aigcfroge = await createAigcfroge({
  port: 0,
})
console.log("✅ Aigcfroge server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await aigcfroge.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new aigcfroge session...")
    const { client, server } = aigcfroge
    ...
    console.log("✅ Created aigcfroge session:", createResult.data.id)
  ...
  console.log("📝 Sending to aigcfroge:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Aigcfroge response:", JSON.stringify(result, null, 2))
