...
const statsHomeTitle = "AI Model Usage Rankings | Aigcfroge Data"
const statsHomeDescription =
  "Explore Aigcfroge Go usage across AI models, including token volume, rankings, market share, token pricing, session cost, cache ratio, and geo breakdowns."
const statsHomeFallbackUrl = "https://aigcfroge.ai/data/"
...
function getStatsHomeUrl(base: string, requestUrl: string) {
  ...
  if (url.hostname === "stats.aigcfroge.ai") return "https://aigcfroge.ai/data/"
  if (url.hostname === "stats.dev.aigcfroge.ai") return "https://dev.aigcfroge.ai/data/"
  return url.toString()
}
  ...
  return (
    <section id="top-models" data-section="top-models">
      <h2 data-slot="top-models-title">
        <strong>Top models.</strong> <span>Usage of models across Aigcfroge Go.</span>
      </h2>
      <Show
...
function UniqueUsersSection(props: { data: StatsHomeData["users"] }) {
  ...
  return (
    <section id="unique-users" data-section="unique-users">
      ...
      <SectionTitle title="Unique Users" description="Daily unique Aigcfroge Go users by model." />
      <Show
        when={data().some((item) => usageTotal(item) > 0)}
        fallback={
