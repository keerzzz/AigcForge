...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve aigcfroge route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "aigcfroge")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "aigcfroge")).toBe("openai")
    expect(statProvider("big-pickle", "", "aigcfroge")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "aigcfroge"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep aigcfroge as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "aigcfroge"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "aigcfroge"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep aigcfroge or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "aigcfroge"), country: "US" })).toMatchObject([
