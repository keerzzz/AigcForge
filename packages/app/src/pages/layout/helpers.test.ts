...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("aigcfroge://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("aigcfroge://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("aigcfroge://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("aigcfroge://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("aigcfroge://open-project")).toBeUndefined()
    expect(parseDeepLink("aigcfroge://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "aigcfroge://open-project?directory=/a",
      "aigcfroge://other?directory=/b",
      "aigcfroge://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("aigcfroge://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("aigcfroge://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("aigcfroge://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("aigcfroge://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "aigcfroge://new-session?directory=/a",
      "aigcfroge://open-project?directory=/b",
      "aigcfroge://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AIGCFROGE__: {
        deepLinks: ["aigcfroge://open-project?directory=/a"],
      },
    } as unknown as Window & { __AIGCFROGE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["aigcfroge://open-project?directory=/a"])
