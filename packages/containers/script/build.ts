...
const reg = process.env.REGISTRY ?? "ghcr.io/keerzzz"
...
const setup = async () => {
  ...
  if (list.includes("aigcfroge")) {
    await $`docker buildx use aigcfroge`
    return
  }
  await $`docker buildx create --name aigcfroge --use`
}
