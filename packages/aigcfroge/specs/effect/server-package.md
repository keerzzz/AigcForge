...
Practical reference for a future `packages/server` split after the aigcfroge
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/aigcfroge`.
...
Until enough shared service code lives outside `packages/aigcfroge`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/aigcfroge`

It should not import `packages/aigcfroge` services while `packages/aigcfroge`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on aigcfroge-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/aigcfroge` implementation details.
