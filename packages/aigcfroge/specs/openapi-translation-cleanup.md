...
Trim `packages/aigcfroge/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/aigcfroge/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/aigcfroge`.
- `[x]` `bun typecheck` from `packages/aigcfroge`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/aigcfroge/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/aigcfroge`.
- `[x]` `bun dev generate > /tmp/aigcfroge-openapi.json` from `packages/aigcfroge`.
...
- Edit `packages/aigcfroge/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/aigcfroge-openapi.json` from `packages/aigcfroge`.
...
- Check source schemas in files like `packages/aigcfroge/src/session/schema.ts`, `packages/aigcfroge/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/aigcfroge/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/aigcfroge/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
