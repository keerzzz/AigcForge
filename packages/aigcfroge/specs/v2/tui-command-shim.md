...
Delete `packages/aigcfroge/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/aigcfroge/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/aigcfroge/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/aigcfroge`
- TUI plugin loader tests in `packages/aigcfroge` if runtime plugin API wiring changed
