...
- Author package entrypoint is `@aigcfroge/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/aigcfroge-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@aigcfroge/plugin/tui`.
- `@aigcfroge/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@aigcfroge/plugin/tui"
...
- The value is a semver range checked against the running Aigcfroge version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.aigcfroge` is absent, no check is performed (backward compatible).
...
- `aigcfroge plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `aigcfroge plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.aigcfroge` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.aigcfroge`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.aigcfroge`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.aigcfroge` dir or `AIGCFROGE_CONFIG_DIR`, Aigcfroge installs `@aigcfroge/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@aigcfroge/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Aigcfroge's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Aigcfroge registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Aigcfroge pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@aigcfroge/plugin/tui`.
...
- `message` is required; `title` defaults to `"aigcfroge"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `aigcfroge.default`.
...
- `api.state` exposes synced TUI state:
  ...
  - `session.todo(sessionID)` (deprecated — legacy projection of the task store, removed in Phase 5)
...
- Local plugins persist installed themes under the local `.aigcfroge/themes` area near the plugin config source.
...
- `--pure` / `AIGCFROGE_PURE` skips external TUI plugins only.
...
- `internal:sidebar-task`
- `internal:sidebar-files`
- `internal:sidebar-footer`
- `internal:plugin-manager`

Sidebar content order is currently: context `100`, mcp `200`, lsp `300`, task `400`, files `500`.
...
- Local smoke plugin: `.aigcfroge/plugins/tui-smoke.tsx`
- Local vim plugin: `.aigcfroge/plugins/tui-vim.tsx`
- Local smoke config: `.aigcfroge/tui.json`
- Local smoke theme: `.aigcfroge/plugins/smoke-theme.json`
