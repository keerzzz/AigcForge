...
const TUI_SCHEMA_URL = "https://aigcfroge.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from aigcfroge.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const aigcfroge = await aigcfrogeFiles(input)
  for (const file of aigcfroge) {
...
async function aigcfrogeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "aigcfroge"),
    ...(await Filesystem.findUp(["aigcfroge.json", "aigcfroge.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "aigcfroge"))
  }
  if (Flag.AIGCFROGE_CONFIG) files.push(Flag.AIGCFROGE_CONFIG)
