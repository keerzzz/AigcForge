import type { PermissionV1 } from "@aigcfroge/core/v1/permission"
import { FSUtil } from "@aigcfroge/core/fs-util"
// CLI entry point for `aigcfroge run` and `aigcfroge --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`aigcfroge --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`aigcfroge --mini --attach`): connects to a running
//      aigcfroge server and runs interactive mode against it.
//
...
import { createAigcfrogeClient, type AigcfrogeClient, type ToolPart } from "@aigcfroge/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<AigcfrogeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run aigcfroge with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running aigcfroge server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AIGCFROGE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AIGCFROGE_SERVER_USERNAME or 'aigcfroge')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: AigcfrogeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: AigcfrogeClient, sessionID: string) {
      ...
      async function current(sdk: AigcfrogeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: AigcfrogeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: AigcfrogeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: AigcfrogeClient) {
        ...
        async function loop(client: AigcfrogeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
