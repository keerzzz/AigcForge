...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall aigcfroge and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Aigcfroge")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g aigcfroge",
      pnpm: "pnpm uninstall -g aigcfroge",
      bun: "bun remove -g aigcfroge",
      yarn: "yarn global remove aigcfroge",
      brew: "brew uninstall aigcfroge",
      choco: "choco uninstall aigcfroge",
      scoop: "scoop uninstall aigcfroge",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "aigcfroge", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Aigcfroge!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# aigcfroge") || content.includes(".aigcfroge/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".aigcfroge/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".aigcfroge/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".aigcfroge"))
    ) {
      continue
    }
