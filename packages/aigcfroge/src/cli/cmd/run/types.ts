// Shared type vocabulary for the direct interactive mode (`aigcfroge --mini`).
//
...
import type { AigcfrogeClient, PermissionRequest, QuestionRequest, ToolPart } from "@aigcfroge/sdk/v2"
import type { TuiConfig } from "@aigcfroge/tui/config"
...
type PromptModel = Parameters<AigcfrogeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<AigcfrogeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<AigcfrogeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<AigcfrogeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<AigcfrogeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<AigcfrogeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<AigcfrogeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<AigcfrogeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<AigcfrogeClient["question"]["reject"]>[0]
