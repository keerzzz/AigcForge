...
import type { LLMClientShape } from "@aigcfroge/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("aigcfroge"))
    return { type: "unsupported", reason: "provider is not openai, aigcfroge, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @aigcfroge/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains aigcfroge-owned. The native runtime only adapts
      // the @aigcfroge/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
