...
export const DEPRECATED_PLUGIN_PACKAGES = ["aigcfroge-openai-codex-auth", "aigcfroge-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, aigcfrogeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(aigcfrogeVersion) || semver.major(aigcfrogeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.aigcfroge
  if (typeof range !== "string") return
  if (!semver.satisfies(aigcfrogeVersion, range)) {
    throw new Error(`Plugin requires aigcfroge ${range} but running ${aigcfrogeVersion}`)
  }
}
