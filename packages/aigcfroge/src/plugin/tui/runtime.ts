...
import { errorData, errorMessage } from "@aigcfroge/tui/util/error"
...
import { hasTheme, upsertTheme } from "@aigcfroge/tui/context/theme"
import { Global } from "@aigcfroge/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@aigcfroge/core/util/flock"
import { Flag } from "@aigcfroge/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@aigcfroge/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@aigcfroge/core/v1/config/plugin"
import { createCommandShim } from "@aigcfroge/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@aigcfroge/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".aigcfroge"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".aigcfroge", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".aigcfroge", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AIGCFROGE_PURE ? [] : pluginOrigins
    if (Flag.AIGCFROGE_PURE && pluginOrigins.length) {
    }
