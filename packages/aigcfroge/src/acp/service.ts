...
import type { Message, AigcfrogeClient, SessionMessageResponse } from "@aigcfroge/sdk/v2"
...
export const AuthMethodID = "aigcfroge-login"
...
export class Service extends Context.Service<Service, Interface>()("@aigcfroge/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `aigcfroge auth login` in the terminal",
      name: "Login with aigcfroge",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: AigcfrogeClient) {
...
function makeUsageService(sdk: AigcfrogeClient) {
...
async function loadDirectorySnapshot(sdk: AigcfrogeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, aigcfroge provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const aigcfrogeProvider = providers[ProviderV2.ID.make("aigcfroge")]
  const aigcfrogeModel = aigcfrogeProvider ? Provider.sort(Object.values(aigcfrogeProvider.models))[0] : undefined
  if (aigcfrogeProvider && aigcfrogeModel) return { providerID: aigcfrogeProvider.id, modelID: aigcfrogeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Aigcfroge service failure", service })
}
