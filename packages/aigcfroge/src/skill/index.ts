...
import { FrontmatterError } from "@aigcfroge/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@aigcfroge/core/util/glob"
...
const AIGCFROGE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with aigcfroge. The model's intuition for what an
// aigcfroge.json should look like is often wrong, and aigcfroge hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch aigcfroge's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, AIGCFROGE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@aigcfroge/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_AIGCFROGE_SKILL_NAME] = {
          name: CUSTOMIZE_AIGCFROGE_SKILL_NAME,
          description: CUSTOMIZE_AIGCFROGE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_AIGCFROGE_SKILL_BODY,
        }
