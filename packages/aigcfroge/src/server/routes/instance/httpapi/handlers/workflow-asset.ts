...
export const workflowAssetHandlers = HttpApiBuilder.group(InstanceHttpApi, "workflow-asset", (handlers) =>
  Effect.gen(function* () {
    ...
    const apply = Effect.fn("WorkflowAssetHttpApi.apply")(function* (ctx: {
      ...
      try {
        relativePath = WorkflowAssetPath.nameToRelativePath(ctx.payload.candidate.name)
      } catch (e) {
        return yield* Effect.fail(
          new InvalidRequestError({ message: `Invalid workflow name: ${e instanceof Error ? e.message : String(e)}` }),
        )
      }
    ...
    const deleteAsset = Effect.fn("WorkflowAssetHttpApi.delete")(function* (ctx: {
      ...
      let relativePath: string
      try {
        relativePath = WorkflowAssetPath.validateRelativePath(ctx.payload.relativePath)
      } catch (e) {
        return yield* Effect.fail(
