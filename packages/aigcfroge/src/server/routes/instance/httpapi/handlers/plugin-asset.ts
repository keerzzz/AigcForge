  ...
  Effect.gen(function* () {
    ...
    const list = Effect.fn("PluginAssetHttpApi.list")(function* (ctx: { query: { search?: string } }) {
      ...
      const bridged = yield* bridgeService
        .scan()
        .pipe(Effect.catch(() => Effect.succeed([] as readonly PluginBridge.BridgeEntry[])))
    ...
    const apply = Effect.fn("PluginAssetHttpApi.apply")(function* (ctx: {
      ...
      try {
        relativePath = PluginAssetPath.nameToRelativePath(ctx.payload.candidate.name)
      } catch (e) {
        return yield* Effect.fail(
          new InvalidRequestError({ message: `Invalid plugin name: ${e instanceof Error ? e.message : String(e)}` }),
        )
      }
      ...
      return yield* locks
        .withLock(registryPath)(
          Effect.uninterruptible(
            Effect.gen(function* () {
              const fileExists = yield* fs.exists(target.canonical).pipe(
                Effect.provide(layer),
              ...
              if (isNew && fileExists && !ctx.payload.overwrite) {
                return yield* Effect.fail(
                  new ConflictError({
                    message: `Already exists at "${registryPath}". Set overwrite=true.`,
              ...
              if (ctx.payload.baseRevision && currentRevision !== ctx.payload.baseRevision) {
                return yield* Effect.fail(
                  new ConflictError({
                    message: `Stale revision for "${registryPath}". Refresh and retry.`,
                    resource: registryPath,
                  }),
                )
              }
              ...
              yield* fileMutation.writeAtomic({ target, content: ctx.payload.candidate.content }).pipe(
                ...
                Effect.catch((err) =>
                  Effect.fail(
                    new InvalidRequestError({ message: `Failed to write at "${registryPath}": ${err.message}` }),
                  ),
                ),
              )
              ...
              const writtenBytes = yield* fs.readFile(target.canonical).pipe(
                ...
                Effect.catch((err) =>
                  Effect.fail(
                    new InvalidRequestError({ message: `Readback failed for "${registryPath}": ${err.message}` }),
                  ),
                ),
              )
              ...
              yield* registry.reload().pipe(
                ...
                Effect.catch((err) =>
                  Effect.gen(function* () {
                    yield* rollback()
                    return yield* Effect.fail(
                      new InvalidRequestError({ message: `Registry reload failed: ${err.message}` }),
                    )
                  }),
                ),
              )

              const readback = yield* registry.getByPath(registryPath).pipe(Effect.provide(layer), Effect.option)
              if (Option.isNone(readback) || readback.value.revision !== writtenRevision) {
                yield* rollback()
                return yield* Effect.fail(
                  new InvalidRequestError({ message: `Readback mismatch for "${registryPath}"` }),
                )
              }
    ...
    const deleteAsset = Effect.fn("PluginAssetHttpApi.delete")(function* (ctx: {
      ...
      let relativePath: string
      try {
        relativePath = PluginAssetPath.validateRelativePath(ctx.payload.relativePath)
      } catch (e) {
        return yield* Effect.fail(
          new InvalidRequestError({ message: `Invalid path: ${e instanceof Error ? e.message : String(e)}` }),
        )
      }
          ...
          Effect.uninterruptible(
            Effect.gen(function* () {
              ...
              const currentBytes = yield* fs.readFile(target.canonical).pipe(
                ...
                Effect.catchReason("PlatformError", "NotFound", () => Effect.succeed(null)),
                Effect.catch((err) =>
                  Effect.fail(new InvalidRequestError({ message: `Read before delete failed: ${err.message}` })),
                ),
              )
              if (currentBytes === null) {
                return yield* Effect.fail(new InvalidRequestError({ message: `Not found: ${relativePath}` }))
              ...
              if (ctx.payload.baseRevision && currentRevision !== ctx.payload.baseRevision) {
                yield* Effect.fail(
                  new ConflictError({
                    message: `Stale revision for "${relativePath}". Refresh and retry.`,
              ...
              yield* fileMutation.remove({ target }).pipe(
                ...
                Effect.catch((err) =>
                  Effect.fail(
                    new InvalidRequestError({ message: `Failed to delete at "${relativePath}": ${err.message}` }),
                  ),
                ),
              )
              ...
              yield* registry.reload().pipe(
                ...
                Effect.catch((err) =>
                  Effect.gen(function* () {
                    ...
                    return yield* Effect.fail(
                      new InvalidRequestError({ message: `Registry reload after delete failed: ${err.message}` }),
                    )
                  }),
                ),
              )
