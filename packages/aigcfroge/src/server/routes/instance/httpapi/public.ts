...
import { AigcfrogeHttpApi } from "./api"
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /delivery/recent limit": { type: "number" },
  "GET /kb limit": { type: "number" },
  "GET /kb/search limit": { type: "number" },
}

const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for aigcfroge serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AigcfrogeHttpApi)
...
export const PublicApi = AigcfrogeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "aigcfroge",
    version: "1.0.0",
    description: "aigcfroge api",
