...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".aigcfroge")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .aigcfroge/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const aigcfroge = path.join(test.directory, ".aigcfroge")
      const tool = path.join(aigcfroge, "tool")
  ...
  it.instance("ignores non-tool exports in .aigcfroge/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".aigcfroge", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-aigcfroge` was registering a tool
  ...
  it.instance("loads tools from .aigcfroge/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const aigcfroge = path.join(test.directory, ".aigcfroge")
      const tools = path.join(aigcfroge, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".aigcfroge", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const aigcfroge = path.join(test.directory, ".aigcfroge")
        const customTools = path.join(aigcfroge, "tools")
        const plugin = path.join(aigcfroge, "node_modules", "@aigcfroge", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(aigcfroge, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@aigcfroge/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".aigcfroge", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(aigcfroge, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@aigcfroge/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(aigcfroge, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(aigcfroge, "node_modules", "cowsay")
