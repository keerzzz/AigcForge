// Subprocess test harness for the aigcfroge CLI. Spawns the real binary against
...
//
// Configuration flows through aigcfroge's built-in test affordances:
//   - AIGCFROGE_CONFIG_CONTENT      : provider config inline, no files to find
//   - AIGCFROGE_TEST_HOME           : pins os.homedir() → tmpdir
//   - AIGCFROGE_DISABLE_PROJECT_CONFIG : skip walking up for aigcfroge.json
//   - AIGCFROGE_PURE                : skip external plugin discovery + install
//   - AIGCFROGE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
const aigcfrogeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(aigcfrogeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    AIGCFROGE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    AIGCFROGE_CONFIG_CONTENT: configJson,
    AIGCFROGE_DISABLE_PROJECT_CONFIG: "1",
    AIGCFROGE_PURE: "1",
    AIGCFROGE_DISABLE_AUTOUPDATE: "1",
    AIGCFROGE_DISABLE_AUTOCOMPACT: "1",
    AIGCFROGE_DISABLE_MODELS_FETCH: "1",
    AIGCFROGE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `aigcfroge run`. New flags should
...
// `aigcfroge serve` is a long-lived process — it never exits on its own.
...
// `aigcfroge acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type AigcfrogeCli = {
  ...
  // Spawn `aigcfroge serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `aigcfroge acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("aigcfroge.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          AIGCFROGE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("aigcfroge.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("aigcfroge.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "aigcfroge server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `aigcfroge serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("aigcfroge.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
      ...
      const proc = yield* Effect.acquireRelease(
        ...
        (p) =>
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() => Promise.resolve(p.stdin.end()))
    ...
    // and hit endpoints on `aigcfroge.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "aigcfroge") {
...
// every call site. Use this for any test that needs the aigcfroge CLI fixture.
//
...
// resources (e.g. `aigcfroge.serve`) without an extra `Effect.scoped` wrapper —
