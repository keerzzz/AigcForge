...
import { LLMEvent, LLMResponse } from "@aigcfroge/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@aigcfroge/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.aigcfroge.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AIGCFROGE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["aigcfroge", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AIGCFROGE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AIGCFROGE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["aigcfroge", "native", "oauth", "tool-loop"],
  ...
  {
    id: "aigcfroge-proxy",
    name: "Aigcfroge proxy",
    providerID: ProviderV2.ID.aigcfroge,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["aigcfroge", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AIGCFROGE_RECORD_CONSOLE_TOKEN && process.env.AIGCFROGE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.aigcfroge,
        name: "Aigcfroge Zen",
        env: ["AIGCFROGE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AIGCFROGE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AIGCFROGE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AIGCFROGE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["aigcfroge", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AIGCFROGE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AIGCFROGE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AIGCFROGE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AIGCFROGE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the aigcfroge LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "aigcfroge.json"),
      JSON.stringify({ $schema: "https://aigcfroge.ai/config.json", ...scenario.config(model) }),
    ),
  )
