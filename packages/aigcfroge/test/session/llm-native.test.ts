...
import { LLMEvent, ToolFailure } from "@aigcfroge/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@aigcfroge/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("aigcfroge"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("aigcfroge") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("aigcfroge") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, aigcfroge, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored aigcfroge auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("aigcfroge") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("aigcfroge"),
