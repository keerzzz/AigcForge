...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const aigcfrogeModel = {
      ...openaiModel,
      providerID: "aigcfroge",
      api: {
        id: "aigcfroge-test",
        url: "https://api.aigcfroge.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, aigcfrogeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.aigcfroge?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.aigcfroge?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          aigcfroge: { itemId: "msg_aigcfroge" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              aigcfroge: { itemId: "msg_aigcfroge_part" },
    ...
    expect(result[0].providerOptions?.aigcfroge?.itemId).toBe("msg_aigcfroge")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.aigcfroge?.itemId).toBe("msg_aigcfroge_part")
