...
import type { HostPluginApi } from "@aigcfroge/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createAigcfrogeClient({
  ...
  let selected = opts.theme?.selected ?? "aigcfroge"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "aigcfroge.default",
