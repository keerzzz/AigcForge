...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ aigcfroge }) =>
    Effect.gen(function* () {
      const result = yield* aigcfroge.spawn(["--replay-limit", "10"])
  ...
  cliIt.live("routes attached sessions to mini mode", ({ aigcfroge }) =>
    Effect.gen(function* () {
      const result = yield* aigcfroge.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ aigcfroge }) =>
    Effect.gen(function* () {
      const result = yield* aigcfroge.spawn(["--mini", "--port", "4096"])
