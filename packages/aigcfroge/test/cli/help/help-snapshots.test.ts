...
// terminal sizes. The default aigcfroge tui command is excluded —
// `aigcfroge --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `aigcfroge --help` prints today;
...
// top-level help on `--help` and exits 1; not a real aigcfroge command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
const SNAPSHOT_ENV = { COLUMNS: "120", LANG: "en", LC_ALL: "en" }

describe("aigcfroge CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const topLevel = yield* aigcfroge.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* aigcfroge.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`aigcfroge ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`aigcfroge ${argv.join(" ")} --help`)
        }
