// Subprocess integration tests for `aigcfroge run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `aigcfroge.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `AIGCFROGE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("aigcfroge run (non-interactive subprocess)", () => {
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aigcfroge.run("say hi")
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aigcfroge.run("use a tool", {
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* aigcfroge.run("think", { extraArgs: ["--thinking"] })
        aigcfroge.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* aigcfroge.run("think again")
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aigcfroge.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aigcfroge.run("say hi", { format: "json" })
        aigcfroge.expectExit(result, 0)

        const events = aigcfroge.parseJsonEvents(result.stdout)
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const result = yield* aigcfroge.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        // Execution of bash requires the build agent (the default meta agent is
        // ask-gated for bash/edit/write, ADR-13 Amendment-2 §1c); this test asserts
        // partial-output preservation, not meta's ask approval flow.
        const result = yield* aigcfroge.run("fail after output", { agent: "build", format: "json" })
  ...
  cliIt.live(
    ...
    ({ home, llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* aigcfroge.run("request permission", { permission: { bash: "ask" } })
        aigcfroge.expectExit(denied, 0)
        ...
        const allowed = yield* aigcfroge.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        aigcfroge.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* aigcfroge.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        aigcfroge.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, aigcfroge }) =>
      Effect.gen(function* () {
        ...
        const server = yield* aigcfroge.serve()

        const result = yield* aigcfroge.run("read the attachment", {
  ...
  cliIt.live(
    ...
    ({ home, aigcfroge }) =>
      Effect.gen(function* () {
        const result = yield* aigcfroge.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, aigcfroge }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* aigcfroge.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
