...
describe("aigcfroge read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // AIGCFROGE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["mcp", "list"])
        aigcfroge.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["providers", "list"])
        aigcfroge.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["models"])
        aigcfroge.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["agent", "list"])
        aigcfroge.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh AIGCFROGE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["session", "list"])
        aigcfroge.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["stats"])
        aigcfroge.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under AIGCFROGE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ aigcfroge }) =>
      Effect.gen(function* () {
        const r = yield* aigcfroge.spawn(["db", "path"])
        aigcfroge.expectExit(r, 0, "db path")
