...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/aigcfroge.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/aigcfroge.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/aigcfroge.jsonc", dir: ".aigcfroge", suggestion: "aigcfroge" },
        expected:
          'Directory ".aigcfroge" in /tmp/aigcfroge.jsonc is not valid. Rename the directory to "aigcfroge" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/aigcfroge.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/aigcfroge.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.aigcfroge.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.aigcfroge.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `aigcfroge models` to list available models",
      "Or check your config (aigcfroge.json) provider/model names",
    ].join("\n")
