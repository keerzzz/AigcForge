...
it.instance("plan agent denies edits except .aigcfroge/plans/*", () =>
  Effect.gen(function* () {
...
it.instance("general agent denies todo tools", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(general, "task_schedule")).toBe("deny")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".aigcfroge", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AIGCFROGE_TEST_HOME
      process.env.AIGCFROGE_TEST_HOME = test.directory
...
it.instance("defaultAgent returns meta when no default_agent config", () =>
  Effect.gen(function* () {
    const agent = yield* load((svc) => svc.defaultAgent())
    expect(agent).toBe("meta")
  }),
)

it.instance("defaultInfo returns resolved meta agent when no default_agent config", () =>
  Effect.gen(function* () {
...
it.instance(
  "defaultAgent returns meta when build is disabled and default_agent not set",
  () =>
    Effect.gen(function* () {
      const agent = yield* load((svc) => svc.defaultAgent())
      // build is disabled, meta is still available as the new default
      expect(agent).toBe("meta")
    }),
  {
    config: {
