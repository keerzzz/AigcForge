...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "aigcfroge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "aigcfroge.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aigcfroge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aigcfroge.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["aigcfroge.json", "aigcfroge.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "aigcfroge.json"),
        path.join(project, "aigcfroge.jsonc"),
      ])
    })
  })
