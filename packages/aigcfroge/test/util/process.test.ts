...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AIGCFROGE_TEST ?? "")'), {
      env: {
        AIGCFROGE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AIGCFROGE_TEST_SHELL"], {
      shell: true,
      env: {
        AIGCFROGE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AIGCFROGE_TEST_SHELL=ok")
  })
