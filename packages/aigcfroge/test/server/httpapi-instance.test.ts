...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-aigcfroge-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.AIGCFROGE_WORKSPACE_ID
      Flag.AIGCFROGE_WORKSPACE_ID = WorkspaceV2.ID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.AIGCFROGE_WORKSPACE_ID = originalWorkspaceID
        }),
      )
      ...
      const response = yield* Effect.promise(() =>
        HttpApiApp.webHandler().handler(
          new Request(`http://localhost/project/${projectID}`, {
            method: "PATCH",
            headers: { "x-aigcfroge-directory": dir, "content-type": "application/json" },
