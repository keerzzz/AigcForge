...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const aigcfrogeFile = path.join(tmp, ".git", "aigcfroge")
      expect(yield* Effect.promise(() => Bun.file(aigcfrogeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "aigcfroge")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "aigcfroge")
      const wrongCache = path.join(parentDir, ".git", "aigcfroge")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "aigcfroge")
      const cacheB = path.join(bareB, "aigcfroge")
      const wrongCache = path.join(parentDir, ".git", "aigcfroge")
