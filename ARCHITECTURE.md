...
| Design in progress | Chat PRD v4.6（资产工作室，已批准 2026-07-18；M1-M7 全部完成 — 7 类资产新建/导入/创建/apply/delete 全闭环）与 Work PRD v4.1（非编程执行层，已批准 2026-07-31，实施计划见 [`docs/roadmap/work-mode-roadmap.md`](docs/roadmap/work-mode-roadmap.md) + [`docs/plan/work-mode-execution-layer-m1.md`](docs/plan/work-mode-execution-layer-m1.md)）；Assistant/My Agents PRDs remain drafts；Custom 第五 Product Mode 处于 ADR/PRD 评审阶段（[`ADR-17`](docs/architecture/adr/ADR-17-custom-mode-composition-platform.md) + [`Custom PRD`](docs/prd/custom-mode-composition-platform.md) + [`Custom roadmap`](docs/roadmap/custom-mode-roadmap.md)），未接受前不得静默改写现有四模式契约 |
...
| Proposed decisions | [`ADR-17`](docs/architecture/adr/ADR-17-custom-mode-composition-platform.md)（Custom 第五 Product Mode、Composition Profile/Plan/Snapshot 与分阶段扩展平台；待 Product/Core/App/Security 评审） |
