# Contributing to Aigcfroge

We want to make it easy for you to contribute to Aigcfroge. Here are the most common type of changes that get merged:
...
- [`help wanted`](https://github.com/keerzzz/AigcForge/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/keerzzz/AigcForge/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/keerzzz/AigcForge/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/keerzzz/AigcForge/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
By default, `bun dev` runs Aigcfroge in the `packages/aigcfroge` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Aigcfroge in the root of the aigcfroge repo itself:
...
```bash
./packages/aigcfroge/script/build.ts --single
```

Then run it with:

```bash
./packages/aigcfroge/dist/aigcfroge-<platform>/bin/aigcfroge
```
...
- Core pieces (the most relevant packages for contributors; the full 21-package topology is in [`ARCHITECTURE.md`](ARCHITECTURE.md) §3):
  ...
  - `packages/plugin`: Source for `@aigcfroge/plugin`

### Understanding bun dev vs aigcfroge

During development, `bun dev` is the local equivalent of the built `aigcfroge` command. Both run the same CLI interface:
...
# Production
aigcfroge --help          # Show all available commands
aigcfroge serve           # Start headless API server
aigcfroge web             # Start server + open web interface
aigcfroge <directory>     # Start TUI in specific directory
```

### Running the API Server

To start the Aigcfroge headless API server:
...
1. **First, start the Aigcfroge server** (see [Running the API Server](#running-the-api-server) section above)
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/aigcfroge/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
The most reliable way to debug Aigcfroge is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
your debugger via that URL. Other methods can result in breakpoints being mapped incorrectly, at least in VSCode (YMMV).

Caveats:

- If you want to run the Aigcfroge TUI and have breakpoints triggered in the server code, you might need to run `bun dev spawn` instead of
  the usual `bun dev`. This is because `bun dev` runs the server in a worker thread and breakpoints might not work there.
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/aigcfroge ./src/index.ts serve --port 4096`,
    then attach TUI with `aigcfroge attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/aigcfroge --conditions=browser ./src/index.ts`
...
- The same problem arises when running Aigcfroge in the VSCode `JavaScript Debug Terminal`
...
- `chore(aigcfroge):` maintenance in the aigcfroge package
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in Aigcfroge. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
...
- **Denounced users** are explicitly blocked. Issues and pull requests from denounced users are automatically closed. If you have been denounced, you can request to be unvouched by reaching out to a maintainer on [Discord](https://aigcfroge.ai/discord)
