...
Move the canonical Aigcfroge terminal application from
`packages/aigcfroge/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/aigcfroge`,
`packages/cli`, or `@aigcfroge/core`.

The SDK is the TUI's Aigcfroge boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/aigcfroge`.
- Do not replace private `packages/aigcfroge` imports with `@aigcfroge/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@aigcfroge/tui`.
...
- Aigcfroge domain data displayed by the TUI
...
```text
packages/aigcfroge/src/cli/cmd/tui
```

Its private dependency on `packages/aigcfroge` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/aigcfroge/src/*`.
...
Status: Completed. The private `@aigcfroge/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@aigcfroge/tui`.
...
- The package has no dependency on `aigcfroge`, `@aigcfroge/cli`, or
  `@aigcfroge/core`.
...
primitives now live in `@aigcfroge/tui`. The legacy host consumes explicit
...
and build inputs through `@aigcfroge/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Aigcfroge globals or process state;
...
Replace process-global Aigcfroge state with resolved TUI inputs.
...
`@aigcfroge/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/aigcfroge` initially.
...
canonical in `@aigcfroge/tui`. Configured references resolve through the new
...
adapters rather than Aigcfroge domain implementations.

Make the SDK the only Aigcfroge domain boundary used by the TUI.
...
- Domain-facing TUI code imports Aigcfroge data and operations only from
  `@aigcfroge/sdk`.
...
`@aigcfroge/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/aigcfroge`.
...
- The package has no imports from `packages/aigcfroge`, `packages/cli`, or
  `@aigcfroge/core`.
...
public `@aigcfroge/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/aigcfroge`.
- Keep the legacy embedded worker and server startup in `packages/aigcfroge`.
- Change those adapters to load config, create transport inputs, and call the
  public `@aigcfroge/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/aigcfroge`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@aigcfroge/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@aigcfroge/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Aigcfroge domain state.
...
cd packages/aigcfroge && bun typecheck
...
rg '@aigcfroge/core|packages/aigcfroge|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/aigcfroge/src packages/aigcfroge/test
```
...
- Build the current-platform `packages/aigcfroge` binary.
