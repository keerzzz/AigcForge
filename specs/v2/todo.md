...
The aigcfroge server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@aigcfroge/llm` tool loop after replacing its
...
- plugins get global "aigcfroge" instance like in that post i showed
- aigcfroge instance has stuff like `aigcfroge.session.prompt()` or
  `aigcfroge.tool.register({...})`
...
- decide whether to preserve deprecated `@aigcfroge/llm` orchestration exports
