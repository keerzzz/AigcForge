# Remove `packages/aigcfroge/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/aigcfroge/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the aigcfroge legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/aigcfroge/src/storage/db.ts` are concentrated in 22 source files:

- `packages/aigcfroge/src/account/repo.ts`
- `packages/aigcfroge/src/cli/cmd/db.ts`
- `packages/aigcfroge/src/cli/cmd/import.ts`
- `packages/aigcfroge/src/cli/cmd/stats.ts`
- `packages/aigcfroge/src/control-plane/workspace.ts`
- `packages/aigcfroge/src/index.ts`
- `packages/aigcfroge/src/node.ts`
- `packages/aigcfroge/src/permission/index.ts`
- `packages/aigcfroge/src/project/project.ts`
- `packages/aigcfroge/src/server/projectors.ts`
- `packages/aigcfroge/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/aigcfroge/src/server/shared/fence.ts`
- `packages/aigcfroge/src/session/message-v2.ts`
- `packages/aigcfroge/src/session/projectors.ts`
- `packages/aigcfroge/src/session/prompt.ts`
- `packages/aigcfroge/src/session/session.ts`
- `packages/aigcfroge/src/session/todo.ts`
- `packages/aigcfroge/src/share/share-next.ts`
- `packages/aigcfroge/src/storage/db.ts`
- `packages/aigcfroge/src/sync/index.ts`
- `packages/aigcfroge/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy aigcfroge database wrapper; `packages/aigcfroge/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the aigcfroge wrapper.
...
Status: Completed. `SyncEvent` and the aigcfroge projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy aigcfroge database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy aigcfroge database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy aigcfroge database wrapper.
...
All migration groups are complete or superseded. `packages/aigcfroge/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No aigcfroge data-migration group remains.

The previous aigcfroge `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate aigcfroge data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/aigcfroge`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/aigcfroge/src`
- `bun typecheck` from `packages/aigcfroge`
- Relevant package tests from `packages/aigcfroge`, not the repo root
