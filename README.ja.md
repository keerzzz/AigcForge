...
# パッケージマネージャー
npm i -g aigcfroge@latest        # bun/pnpm/yarn でもOK
scoop install aigcfroge             # Windows
choco install aigcfroge             # Windows
brew install anomalyco/tap/aigcfroge # macOS と Linux（推奨。常に最新）
brew install aigcfroge              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S aigcfroge            # Arch Linux (Stable)
paru -S aigcfroge-bin               # Arch Linux (Latest from AUR)
mise use -g aigcfroge               # どのOSでも
nix run nixpkgs#aigcfroge           # または github:keerzzz/AigcForge で最新 dev ブランチ
```
...
Aigcfroge はデスクトップアプリとしても利用できます。[releases page](https://github.com/keerzzz/AigcForge/releases) から直接ダウンロードするか、[aigcfroge.ai/download](https://aigcfroge.ai/download) を利用してください。
...
1. `$AIGCFROGE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.aigcfroge/bin` - デフォルトのフォールバック
...
Aigcfroge には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://aigcfroge.ai/docs/agents) の詳細はこちら。

### ドキュメント

Aigcfroge の設定については [**ドキュメント**](https://aigcfroge.ai/docs) を参照してください。

### コントリビュート

Aigcfroge に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Aigcfroge の上に構築する

Aigcfroge に関連するプロジェクトで、名前に "aigcfroge"（例: "aigcfroge-dashboard" や "aigcfroge-mobile"）を含める場合は、そのプロジェクトが Aigcfroge チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/aigcfroge) | [X.com](https://x.com/aigcfroge)
